...
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Gestione Settimanale e Storico Assenze (A.S. 2026/2027 - v25)")
...
with tab_assenze:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            tc3.write(ind_val)
            tc4.write(item['Tipo'])
            d_nome_d = item['Docente']
            r_doc_d = df_schedule[df_schedule['Docente'] == d_nome_d]
            r_info_d = r_doc_d.iloc[0] if len(r_doc_d) > 0 else {}
            ore_sost_d = [s for s in item.get('Ore', []) if str(r_info_d.get(f"{giorno_selezionato}_{s}", '')).strip().upper() not in ['DISP', 'D', 'DISPOSIZIONE', '', 'NAN', 'NONE']]
            ore_str = ", ".join([o.split(" ")[0] for o in ore_sost_d]) if ore_sost_d else "Nessuna classe"
            tc5.write(ore_str)
    ...
    for g in GIORNI:
        for item in st.session_state.assenze_per_giorno.get(g, []):
            d_nome_w = item['Docente']
            r_doc_w = df_schedule[df_schedule['Docente'] == d_nome_w]
            r_info_w = r_doc_w.iloc[0] if len(r_doc_w) > 0 else {}
            ore_sost_w = [s for s in item.get('Ore', []) if str(r_info_w.get(f"{g}_{s}", '')).strip().upper() not in ['DISP', 'D', 'DISPOSIZIONE', '', 'NAN', 'NONE']]
            ore_str_w = ", ".join([o.split(" ")[0] for o in ore_sost_w]) if ore_sost_w else "Nessuna classe"
            ind_val = item.get('Indirizzo', '')
            if not ind_val:
                if len(r_doc_w) > 0:
                    ind_val = str(r_doc_w.iloc[0].get('Indirizzo', ''))
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        out_giorno = io.BytesIO()
        with pd.ExcelWriter(out_giorno, engine='openpyxl') as writer:
            df_reg_giorno.to_excel(writer, index=False, sheet_name=f'Sostituzioni {giorno_selezionato}')
        excel_giorno_data = out_giorno.getvalue()
        
        st.download_button(
            label=f"📥 Scarica Report Sostituzioni di Oggi (Excel .xlsx)",
            data=excel_giorno_data,
            file_name=f"sostituzioni_{giorno_selezionato}_Calvino.xlsx",
    ...
    if st.session_state.registro:
        ...
        with pd.ExcelWriter(out_completo, engine='openpyxl') as writer:
            df_reg_completo.to_excel(writer, index=False, sheet_name='Storico Settimanale')
        excel_completo_data = out_completo.getvalue()
        
        col_dl1, col_dl2 = st.columns([3, 1])
        with col_dl1:
            st.download_button(
                label="📥 Scarica Report Storico Completo Settimanale (Excel .xlsx)",
                data=excel_completo_data,
                file_name="sostituzioni_storico_completo_Calvino.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
        with col_dl2:
