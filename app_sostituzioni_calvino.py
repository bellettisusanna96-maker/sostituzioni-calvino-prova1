...
def is_docente_assente_in_ora(docente_nome, giorno, ora):
    if not docente_nome or str(docente_nome).strip() in ["🔴 IN SOSPESO", "", "None", "nan"]:
        return False
    doc_clean = str(docente_nome).strip().upper()
    for item in st.session_state.assenze_per_giorno.get(giorno, []):
        if str(item.get('Docente', '')).strip().upper() == doc_clean:
...
def get_conflitti_supplenti_assenti(giorno=None):
    conflitti = []
    for reg in st.session_state.registro:
        g = reg.get('Giorno')
        if giorno and g != giorno:
            continue
        sost = reg.get('Docente Sostituto')
        ora = reg.get('Ora')
        cls = reg.get('Classe')
        d_ass = reg.get('Docente Assente')
        if is_docente_assente_in_ora(sost, g, ora):
            conflitti.append({
                'Giorno': g,
                'Ora': ora,
                'Classe': cls,
                'Docente Assente': d_ass,
                'Supplente Assente': sost
            })
    return conflitti
...
# Calcolo riassuntivo per la sidebar
tot_ass_sett = sum(len(st.session_state.assenze_per_giorno.get(g, [])) for g in GIORNI)
tot_sost_sett = len(st.session_state.registro)
conflitti_totali = get_conflitti_supplenti_assenti()

st.sidebar.markdown(f"📊 **Stato Attuale Memoria:**")
st.sidebar.markdown(f"• Docenti assenti inseriti: **{tot_ass_sett}**")
st.sidebar.markdown(f"• Supplenze confermate: **{tot_sost_sett}**")
if conflitti_totali:
    st.sidebar.error(f"🚨 **CONFLITTI ATTIVI: {len(conflitti_totali)}** (Supplenti assenti)")
...
uploaded_backup = st.sidebar.file_uploader("Ripristina da file Backup (JSON)", type=["json"], key="uploader_backup_sidebar")
...
# Banner di avviso globale se ci sono conflitti attivi nella sessione
if conflitti_totali:
    st.error(f"🚨 **ALLERTA CRITICA ({len(conflitti_totali)} CONFLITTO/I DETECTATO/I)**: Ci sono docenti assegnati in supplenza che risultano ASSENTI! Vai nel **Tab 2** per riassegnare un nuovo sostituto disponibile.")
...
with tab_assenze:
    ...
    # Controlla se il docente selezionato era GIÀ stato assegnato in supplenza
    supplenze_precedenti = [
        r for r in st.session_state.registro 
        if r.get('Giorno') == giorno_selezionato and str(r.get('Docente Sostituto', '')).strip().upper() == str(doc_ass_sel).strip().upper()
    ]
    ...
    if supplenze_precedenti:
        dettaglio_supp = [f"• **{r['Ora']}**: Classe `{r['Classe']}` (Sostituiva {r['Docente Assente']})" for r in supplenze_precedenti]
        st.error(f"🚨 **AVVISO CONFLITTO:** Il docente **{doc_ass_sel}** risulta già assegnato come **SUPPLENTE** per {len(supplenze_precedenti)} classe/i il **{giorno_selezionato}**:\n\n" + "\n".join(dettaglio_supp) + "\n\nSe lo aggiungi agli assenti, quelle supplenze risulteranno scoperte e dovranno essere riassegnate nel **TAB 2**!")
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Controlla se il docente era anche assegnato come supplente
            was_suppl = any(
                r.get('Giorno') == giorno_selezionato and str(r.get('Docente Sostituto', '')).strip().upper() == str(item['Docente']).strip().upper()
                for r in st.session_state.registro
            )
            
            if was_suppl:
                tc1.markdown(f"👤 **{item['Docente']}**  \n🚨 *Era registrato come supplente!*")
            else:
                tc1.markdown(f"👤 **{item['Docente']}**")
            ...
            if tc6.button("❌", key=f"btn_remove_inline_tab1_{idx}_{item['Docente']}", help=f"Rimuovi {item['Docente']} dagli assenti"):
                ...
                save_persistent_data()
                st.success(f"Rimesso in servizio **{doc_removed}**!")
                st.rerun()
    ...
    # Tabella riassuntiva di TUTTE le assenze della settimana
    st.markdown("---")
...
with tab_sostituzioni:
    ...
    # Mostra un avviso di emergenza specifico se ci sono conflitti in questa giornata
    conflitti_giorno = get_conflitti_supplenti_assenti(giorno_selezionato)
    if conflitti_giorno:
        st.error(f"🚨 **ATTENZIONE:** Ci sono **{len(conflitti_giorno)} supplenza/e in CONFLITTO** per {giorno_selezionato} perché il docente sostituto assegnato risulta **ASSENTE**!")
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        th1, th2, th3, th4, th5, th6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.5, 1.0])
        th1.markdown("**Ora**")
        th2.markdown("**Classe**")
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            tc1, tc2, tc3, tc4, tc5, tc6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.5, 1.0])
            ...
            if sost_val != "🔴 IN SOSPESO":
                if is_docente_assente_in_ora(sost_val, giorno_selezionato, ora_row):
                ...
                if tc6.button("❌", key=f"btn_del_inline_{idx_row}_{row_s['Ora']}", help=f"Rimuovi sostituto {sost_val}"):
                    ...
                    save_persistent_data()
                    st.success(f"Rimosso sostituto per la classe {row_s['Classe / Slot']}!")
                    st.rerun()
            else:
        ...
        st.markdown("---")
        st.subheader("🔍 Assegnazione / Riassegnazione Sostituto per un'Ora Scoperta")
        
        opzioni_scoperte = []
        for idx, row_s in enumerate(classi_scoperte_lista):
            s_val = row_s['Sostituto Assegnato']
            o_val = row_s['Ora']
            if s_val == "🔴 IN SOSPESO":
                stato_icon = "🔴"
            elif is_docente_assente_in_ora(s_val, giorno_selezionato, o_val):
                stato_icon = "🚨 CONFLITTO (Supplente Assente)"
            else:
                stato_icon = "🟢"
            opzioni_scoperte.append(f"{stato_icon} {o_val} | Assente: {row_s['Docente Assente']} | Classe: {row_s['Classe / Slot']} | Sostituto: {s_val}")
        ...
        # In evidenza se c'è un conflitto attivo
        if sostituto_corrente != "🔴 IN SOSPESO" and is_docente_assente_in_ora(sostituto_corrente, giorno_selezionato, ora_target):
            st.error(f"🚨 **CONFLITTO DA RISOLVERE:** Il sostituto precedentemente assegnato (**{sostituto_corrente}**) risulta **ASSENTE** per {giorno_selezionato} ({ora_target}). Seleziona un nuovo sostituto dall'elenco sottostante.")
        else:
        ...
        # Calcolo rigoroso docenti esclusi per l'ora target
        esclusi_ora = set()
        
        # 1. Chiunque sia assente nell'ora target (sia per orario parziale che per tutto il giorno)
        for d_check in df_schedule['Docente'].unique():
            if is_docente_assente_in_ora(d_check, giorno_selezionato, ora_target):
                esclusi_ora.add(d_check)
                
        # 2. Chiunque sia già occupato come sostituto in quell'ora per un'altra classe
        for reg in st.session_state.registro:
            if reg.get('Giorno') == giorno_selezionato and reg.get('Ora') == ora_target:
                s_already = reg.get('Docente Sostituto')
                if s_already and s_already != "🔴 IN SOSPESO" and reg.get('Docente Assente') != docente_assente_target:
                    esclusi_ora.add(s_already)
        
        st.caption(f"🛡️ **Sicurezza Anti-Sovrapposizione ({ora_target}):** {len(esclusi_ora)} docenti esclusi perché assenti o già impegnati in supplenza.")
        ...
        for idx, r in df_schedule.iterrows():
            ...
            if d_nome in esclusi_ora:
                continue
            
            # Escludi chi non è in servizio il giorno selezionato
        ...
        if all_cands:
            ...
            btn_label = "✅ Riassegna Nuovo Sostituto" if (sostituto_corrente != "🔴 IN SOSPESO") else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        # Aggiunta colonna visiva per evidenziare se il sostituto è in conflitto
        reg_enhanced = []
        for r in reg_giorno_sorted:
            r_copy = dict(r)
            sost = r_copy.get('Docente Sostituto')
            ora = r_copy.get('Ora')
            if is_docente_assente_in_ora(sost, giorno_selezionato, ora):
                r_copy['Stato Sostituto'] = f"🚨 ASSENTE ({sost})"
            else:
                r_copy['Stato Sostituto'] = "🟢 OK"
            reg_enhanced.append(r_copy)
            
        df_reg_giorno = pd.DataFrame(reg_enhanced)
    ...
    if st.session_state.registro:
        ...
        for r in reg_completo_sorted:
            ...
            if is_docente_assente_in_ora(sost, g, ora):
                ...
                r_copy['Stato Sostituto'] = "🟢 OK"
            reg_completo_enhanced.append(r_copy)
            
        df_reg_completo = pd.DataFrame(reg_completo_enhanced)
