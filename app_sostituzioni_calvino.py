...
# Assicuriamoci che tutti i giorni siano presenti
for g in GIORNI:
...
uploaded_backup = st.sidebar.file_uploader("Ripristina da file Backup (JSON)", type=["json"], key="backup_uploader_widget")

if uploaded_backup is not None:
    file_id = f"{uploaded_backup.name}_{uploaded_backup.size}"
    if st.session_state.last_uploaded_file != file_id:
        try:
            ...
            st.sidebar.success("✅ Sessione ripristinata con successo dal file JSON!")
            st.rerun()
        except Exception as e:
            st.sidebar.error(f"Errore ripristino JSON: {e}")
...
# CALCOLO GLOBALE CONFLITTI PER IL GIORNO SELEZIONATO
# Un docente è assente in un'ora se è registrato tra gli assenti per quell'ora O per "Tutto il giorno"
docenti_assenti_totali_per_ora = {slot: set() for slot in ORE}
for item in st.session_state.assenze_per_giorno.get(giorno_selezionato, []):
    d_nome = item['Docente']
    t_tipo = item.get('Tipo', '')
    if t_tipo == "🔴 Tutto il Giorno":
        for slot in ORE:
            docenti_assenti_totali_per_ora[slot].add(d_nome)
    else:
        for slot in item.get('Ore', []):
            if slot in docenti_assenti_totali_per_ora:
                docenti_assenti_totali_per_ora[slot].add(d_nome)
...
for reg in st.session_state.registro:
    if reg.get('Giorno') == giorno_selezionato:
        s_ora = reg.get('Ora')
        s_supplente = reg.get('Docente Sostituto')
        if s_supplente in docenti_assenti_totali_per_ora.get(s_ora, set()):
            conflitti_attivi.append({
                'Giorno': giorno_selezionato,
                'Ora': s_ora,
                'Classe': reg.get('Classe'),
                'Docente Assente Iniziale': reg.get('Docente Assente'),
                'Supplente Assente': s_supplente
            })

if conflitti_attivi:
    dettagli_conf = [
        f"• **{c['Ora']}** | Classe `{c['Classe']}`: Il supplente **{c['Supplente Assente']}** risulta ora **ASSENTE** (Sostituiva *{c['Docente Assente Iniziale']}*)"
        for c in conflitti_attivi
    ]
    st.error(
        f"🚨 **ALLERTA SOSPESA CRITICA ({len(conflitti_attivi)} CONFLITTO/I RILEVATO/I PER {giorno_selezionato.upper()}):**\n" +
        "Ci sono docenti che erano stati assegnati come SUPPLENTI ma che sono stati successivamente segnati come ASSENTI!\n\n" +
        "\n".join(dettagli_conf) +
        "\n\n👉 **Azione Richiesta:** Vai nel **TAB 2 (Quadro Ore Scoperte)** per scegliere e riassegnare un nuovo sostituto disponibile!"
    )
...
with tab_assenze:
    ...
    with col_add3:
        ...
        # Verifica se il docente selezionato è già stato assegnato come SUPPLENTE per il giorno selezionato
        ...
        # AVVISO CRITICO IN EVIDENZA SE IL DOCENTE È GIÀ STATO ASSEGNATO COME SUPPLENTE
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Badge se era supplente
            is_supplente_badge = ""
            for reg in st.session_state.registro:
                if reg.get('Giorno') == giorno_selezionato and reg.get('Docente Sostituto') == item['Docente']:
                    is_supplente_badge = " ⚠️ *(Era registrato come supplente!)*"
                    break
                    
            tc1.markdown(f"👤 **{item['Docente']}**{is_supplente_badge}")
...
with tab_sostituzioni:
    ...
    docenti_assenti_per_ora = docenti_assenti_totali_per_ora
    ...
    for reg in st.session_state.registro:
        if reg.get('Giorno') == giorno_selezionato:
            s_ora = reg.get('Ora')
            s_supplente = reg.get('Docente Sostituto')
            # Considera occupato solo se il supplente NON è a sua volta assente
            if s_supplente not in docenti_assenti_per_ora.get(s_ora, set()):
                docenti_sostituti_occupati_per_ora[s_ora].add(s_supplente)
    ...
    for item in st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # Filtra le ore di assenza: se Tutto il giorno, prendi solo le ore in cui il docente ha lezioni o DISP
        ore_da_controllare = item['Ore']
        if item.get('Tipo') == "🔴 Tutto il Giorno":
            ore_da_controllare = [s for s in ORE if str(r_info.get(f"{giorno_selezionato}_{s}", '')).strip().upper() not in ['NAN', 'NONE', '']]
            
        for slot in ore_da_controllare:
            ...
            if cls_val and cls_val.upper() != 'NAN' and cls_val != '':
                if cls_val.upper() in ['DISP', 'D', 'DISPOSIZIONE']:
                    ...
                    if sostituto_assegnato:
                        if sostituto_assegnato in docenti_assenti_per_ora.get(slot, set()):
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            ...
            if "CONFLITTO" in sost_val:
                ...
                if tc6.button("❌", key=f"btn_del_inline_conf_{idx_row}_{row_s['Ora']}", help="Rimuovi supplenza in conflitto"):
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            if "CONFLITTO" in row_s['Sostituto Assegnato']:
                stato_icon = "🚨"
        ...
        if "CONFLITTO" in sostituto_corrente:
            st.error(f"🚨 **CONFLITTO DA RISOLVERE:** La classe `{classe_target}` ({ora_target}) era stata assegnata a `{sostituto_corrente}`, che ora è ASSENTE. Scegli un nuovo sostituto qui sotto.")
        else:
        ...
        if all_cands:
            ...
            btn_label = "✅ Riassegna Nuovo Sostituto" if "CONFLITTO" in sostituto_corrente else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        # Aggiunta colonna Stato Sostituto se assente
        stati_sost = []
        for _, r in df_reg_giorno.iterrows():
            suppl = r.get('Docente Sostituto')
            g_item = r.get('Giorno')
            o_item = r.get('Ora')
            if suppl in docenti_assenti_totali_per_ora.get(o_item, set()):
                stati_sost.append(f"🚨 ASSENTE ({suppl})")
            else:
                stati_sost.append("🟢 Regolare")
        df_reg_giorno['Stato Sostituzione'] = stati_sost
