...
def generate_pdf_circolare(giorno_sel, reg_giorno, assenze_giorno):
    pdf = FPDF(orientation='P', unit='mm', format='A4')
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)
    
    pdf.set_font("Helvetica", 'B', 16)
    pdf.cell(0, 8, "ISTITUTO D'ISTRUZIONE SUPERIORE \"ITALO CALVINO\"", ln=True, align='C')
    pdf.set_font("Helvetica", 'I', 11)
    pdf.cell(0, 6, f"Circolare Giornaliera Sostituzioni e Supplenze - {giorno_sel}", ln=True, align='C')
    pdf.ln(5)
    
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 7, "1. Docenti Assenti / In Permesso", ln=True)
    pdf.set_font("Helvetica", 'B', 10)
    pdf.set_fill_color(230, 230, 230)
    
    pdf.cell(50, 7, "Docente Assente", 1, 0, 'L', True)
    pdf.cell(40, 7, "Materia", 1, 0, 'L', True)
    pdf.cell(35, 7, "Indirizzo", 1, 0, 'L', True)
    pdf.cell(65, 7, "Tipologia", 1, 1, 'L', True)
    
    pdf.set_font("Helvetica", '', 10)
    if not assenze_giorno:
        pdf.cell(190, 7, "Nessun docente assente registrato.", 1, 1, 'C')
    else:
        for a in assenze_giorno:
            pdf.cell(50, 6, str(a.get('Docente', ''))[:25], 1)
            pdf.cell(40, 6, str(a.get('Materia', ''))[:20], 1)
            pdf.cell(35, 6, str(a.get('Indirizzo', ''))[:18], 1)
            pdf.cell(65, 6, str(a.get('Tipo', ''))[:35], 1, 1)
            
    pdf.ln(5)
    pdf.set_font("Helvetica", 'B', 12)
    pdf.cell(0, 7, "2. Sostituzioni Assegnate del Giorno", ln=True)
    
    pdf.set_font("Helvetica", 'B', 10)
    pdf.cell(20, 7, "Ora", 1, 0, 'C', True)
    pdf.cell(25, 7, "Classe", 1, 0, 'C', True)
    pdf.cell(45, 7, "Docente Assente", 1, 0, 'L', True)
    pdf.cell(50, 7, "Docente Sostituto", 1, 0, 'L', True)
    pdf.cell(50, 7, "Materia", 1, 1, 'L', True)
    
    pdf.set_font("Helvetica", '', 10)
    if not reg_giorno:
        pdf.cell(190, 7, "Nessuna sostituzione registrata per questo giorno.", 1, 1, 'C')
    else:
        for r in reg_giorno:
            pdf.cell(20, 6, str(r.get('Ora', '')).split(' ')[0], 1, 0, 'C')
            pdf.cell(25, 6, str(r.get('Classe', '')), 1, 0, 'C')
            pdf.cell(45, 6, str(r.get('Docente Assente', ''))[:22], 1)
            pdf.cell(50, 6, str(r.get('Docente Sostituto', ''))[:25], 1)
            pdf.cell(50, 6, str(r.get('Materia', ''))[:25], 1, 1)
            
    pdf.ln(10)
    pdf.set_font("Helvetica", 'I', 9)
    pdf.cell(0, 5, "Documento generato dal Sistema di Gestione Sostituzioni IIS Calvino", align='R')
    
    return bytes(pdf.output())

def generate_pdf_registro(reg_completo):
    ...
    pdf.cell(0, 8, "ISTITUTO D'ISTRUZIONE SUPERIORE \"ITALO CALVINO\"", ln=True, align='C')
    pdf.set_font("Helvetica", 'I', 11)
    pdf.cell(0, 6, "Registro Storico Sostituzioni", ln=True, align='C')
    pdf.ln(5)
    
    pdf.set_font("Helvetica", 'B', 9)
    pdf.set_fill_color(230, 230, 230)
    
    pdf.cell(25, 7, "Giorno", 1, 0, 'C', True)
    pdf.cell(18, 7, "Ora", 1, 0, 'C', True)
    pdf.cell(22, 7, "Classe", 1, 0, 'C', True)
    pdf.cell(50, 7, "Docente Assente", 1, 0, 'L', True)
    pdf.cell(50, 7, "Docente Sostituto", 1, 0, 'L', True)
    pdf.cell(42, 7, "Materia", 1, 0, 'L', True)
    pdf.cell(70, 7, "Priorita Applicata", 1, 1, 'L', True)
    
    pdf.set_font("Helvetica", '', 9)
    if not reg_completo:
        pdf.cell(277, 7, "Nessun dato presente nel registro.", 1, 1, 'C')
    else:
        for r in reg_completo:
            pdf.cell(25, 6, str(r.get('Giorno', '')), 1, 0, 'C')
            pdf.cell(18, 6, str(r.get('Ora', '')).split(' ')[0], 1, 0, 'C')
            pdf.cell(22, 6, str(r.get('Classe', '')), 1, 0, 'C')
            pdf.cell(50, 6, str(r.get('Docente Assente', ''))[:26], 1)
            pdf.cell(50, 6, str(r.get('Docente Sostituto', ''))[:26], 1)
            pdf.cell(42, 6, str(r.get('Materia', ''))[:22], 1)
            pdf.cell(70, 6, str(r.get('Priorita Applicata', ''))[:38], 1, 1)
            
    pdf.ln(8)
    pdf.set_font("Helvetica", 'I', 8)
    pdf.cell(0, 5, "IIS Italo Calvino - Registro Sostituzioni", align='R')
...
with tab_assenze:
    ...
    if df_schedule.empty or 'Docente' not in df_schedule.columns:
        ...
        # Box azzurro con orario di servizio del docente selezionato
        if docente_ass_sel:
            orario_items = []
            for slot in ORE:
        ...
        if not curr_assenze:
            ...
            for idx_a, item_a in enumerate(curr_assenze):
                ...
                # Calcolo delle sole ore con classe da sostituire
