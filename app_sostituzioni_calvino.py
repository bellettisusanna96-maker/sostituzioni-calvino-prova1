...
with tab_assenze:
    ...
    if df_schedule.empty or 'Docente' not in df_schedule.columns:
        ...
        with c_ins2:
            tipo_ass_sel = st.radio("Tipologia Assenza:", ["🔴 Tutto il Giorno", "⏱️ Ore Specifiche"], key=f"radio_tipo_ass_{giorno_selezionato}")

        ore_selezionate = []
        with c_ins3:
            if tipo_ass_sel == "⏱️ Ore Specifiche":
        ...
        if st.button("➕ Registra Assenza Docente", type="primary", key=f"btn_add_ass_{giorno_selezionato}"):
            ...
            elif tipo_ass_sel == "⏱️ Ore Specifiche" and not ore_selezionate:
        ...
        if not curr_assenze:
            ...
            for idx_a, item_a in enumerate(curr_assenze):
                ca1, ca2, ca3, ca4, ca5 = st.columns([2.5, 2.0, 2.0, 2.0, 1.0])
                ca1.write(f"👤 **{item_a['Docente']}**")
                ca2.write(f"📚 {item_a.get('Materia', '-')}")
                ca3.write(f"🏫 {item_a.get('Indirizzo', '-')}")
                ca4.write(f"{item_a.get('Tipo', '-')} ({len(item_a.get('Ore', []))} ore)")
                if ca5.button("🗑️", key=f"btn_del_ass_{idx_a}_{giorno_selezionato}"):
...
with tab_sostituzioni:
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # Linea Sicurezza Anti-Sovrapposizione rimossa
