    ...
    if df_schedule.empty or 'Docente' not in df_schedule.columns:
        ...
        if not curr_assenze:
            ...
            for idx_a, item_a in enumerate(curr_assenze):
                ...
                ore_list = item_a.get('Ore', [])
                if item_a.get('Tipo') == "🔴 Tutto il Giorno" or len(ore_list) == len(ORE):
                    ore_str = "Tutto il giorno (1ª-7ª)"
                else:
                    ore_short = [o.split(' ')[0] for o in ore_list]
                    ore_str = ", ".join(ore_short) if ore_short else "-"
