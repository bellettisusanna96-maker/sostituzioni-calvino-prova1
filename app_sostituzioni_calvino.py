...
def create_circolare_pdf_bytes(giorno, data_str, num_circolare, rows, oggetto_str=""):
    ...
    intro_txt = f"Si comunicano le variazioni d'orario e le sostituzioni dei docenti assenti per la giornata di {giorno} ({data_str}). I docenti sostituti sono tenuti a firmare la presenza in classe sul Registro Elettronico."
...
with tab_sostituzioni:
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # CSS per lo sfondo azzurro chiaro della tendina (anche chiusa) e azzurro scuro/blu su hover e selezione
        st.markdown("""
        <style>
        /* Campo principale della selectbox chiusa (la prima riga in alto) */
        ...
        /* Lista del menu a comparsa (popover dropdown) */
        ...
        /* Effetto Passaggio Cursore (Hover) e Selezione -> Azzurro Scuro / Blu con testo bianco */
...
with tab_registro:
    ...
    # FILTRI DI RICERCA RAPIDA NEL TAB 3
    st.markdown("#### 🔍 Filtri di Ricerca Rapida nello Storico")
    
    reg_all_raw = st.session_state.get('registro', [])
    
    # Estrazione liste opzioni per i filtri
    classi_disp = sorted(list({str(r.get('Classe', '')).strip() for r in reg_all_raw if str(r.get('Classe', '')).strip()}))
    docenti_disp = sorted(list({str(r.get('Docente Sostituto', '')).strip() for r in reg_all_raw if str(r.get('Docente Sostituto', '')).strip() and str(r.get('Docente Sostituto', '')).strip() != '-'}))
    
    f_col1, f_col2, f_col3 = st.columns([1.5, 2.0, 1.5])
    with f_col1:
        filtro_classe = st.selectbox("🏫 Filtra per Classe:", ["Tutte le Classi"] + classi_disp, key="tab3_filter_classe")
    with f_col2:
        filtro_docente = st.selectbox("👤 Filtra per Docente Sostituto:", ["Tutti i Docenti"] + docenti_disp, key="tab3_filter_docente")
    with f_col3:
        filtro_tipo = st.selectbox("📋 Filtra per Tipologia:", ["Tutte le Tipologie", "DISP (A Disposizione)", "EXTRA (Ora Buca / Straordinario)"], key="tab3_filter_tipo")

    # Applicazione dei filtri al registro
    reg_filtrato = []
    for r in reg_all_raw:
        c_val = str(r.get('Classe', '')).strip()
        d_val = str(r.get('Docente Sostituto', '')).strip()
        t_val = str(r.get('Tipologia Sostituzione', '')).strip().upper()

        if filtro_classe != "Tutte le Classi" and c_val != filtro_classe:
            continue
        if filtro_docente != "Tutti i Docenti" and d_val != filtro_docente:
            continue
        if filtro_tipo == "DISP (A Disposizione)" and t_val != "DISP":
            continue
        if filtro_tipo == "EXTRA (Ora Buca / Straordinario)" and t_val != "EXTRA":
            continue
        reg_filtrato.append(r)

    # Box informativo statistico se è selezionato un singolo docente
    if filtro_docente != "Tutti i Docenti":
        tot_coperte = len(reg_filtrato)
        disp_cnt = sum(1 for r in reg_filtrato if str(r.get('Tipologia Sostituzione', '')).strip().upper() == "DISP")
        extra_cnt = sum(1 for r in reg_filtrato if str(r.get('Tipologia Sostituzione', '')).strip().upper() == "EXTRA")
        
        # Conteggio assenze del docente selezionato
        tot_assenze_doc = 0
        for g_k, items_g in st.session_state.get('assenze_per_giorno', {}).items():
            for item_a in items_g:
                if item_a.get('Docente') == filtro_docente:
                    tot_assenze_doc += 1

        st.info(f"📊 **Statistiche per il Prof. {filtro_docente}:** **{tot_coperte}** supplenze coperte in totale (🟢 **{disp_cnt}** in ora DISP | 🔵 **{extra_cnt}** in ora EXTRA) | 🔴 **{tot_assenze_doc}** giornate/permessi di assenza registrati.")

    reg_tot = sort_registro_by_day_and_teacher(reg_filtrato)
    rows_display_tot = []
    for r in reg_tot:
        d_sost = r.get('Docente Sostituto', '')
        g_val = r.get('Giorno', '')
    ...
    with col_btn_t1:
        if not df_tot.empty:
            ...
            st.download_button(
                label="📥 Scarica Report Storico Filtrato (CSV)",
                data=csv_tot,
                file_name="Report_Storico_Sostituzioni_Settimanale.csv",
                mime="text/csv"
            )
        else:
            st.button("📥 Scarica Report Storico Filtrato (CSV)", disabled=True)

    with col_btn_t2:
        st.download_button(
            label="📄 Scarica Report Storico Filtrato (PDF)",
            data=pdf_bytes_tot,
