...
### Inizializzazione Session State con caricamento persistente
p_data = load_persistent_data()
if 'assenze_per_giorno' not in st.session_state:
    st.session_state.assenze_per_giorno = p_data.get("assenze_per_giorno", {g: [] for g in GIORNI})
...
tot_ass_mem = sum(len(items) for items in st.session_state.assenze_per_giorno.values())
tot_reg_mem = len(st.session_state.registro)

st.sidebar.info(f"📊 **Stato In Memoria:**\n• Docenti Assenti: **{tot_ass_mem}**\n• Sostituzioni Assegnate: **{tot_reg_mem}**")
...
if uploaded_backup is not None:
    try:
        file_content = uploaded_backup.getvalue().decode("utf-8")
        b_data = json.loads(file_content)
        ...
        st.sidebar.success("✅ Sessione ripristinata con successo!")
        st.rerun()
    except Exception as e:
...
# ==============================================================================
# VERIFICA CONFLITTI GLOBALI: SUPPLENTI ASSEGNATI CHE SONO POI RISULTATI ASSENTI
# ==============================================================================
conflitti_supplenti_assenti = []
for reg in st.session_state.registro:
    g_reg = reg.get('Giorno')
    ora_reg = reg.get('Ora')
    supp_reg = reg.get('Docente Sostituto')
    
    # Verifica se questo supplente è tra gli assenti per quel giorno e quella specifica ora
    assenti_quel_giorno = st.session_state.assenze_per_giorno.get(g_reg, [])
    is_supp_assente = False
    for a_item in assenti_quel_giorno:
        if a_item.get('Docente') == supp_reg:
            # Se è assente tutto il giorno oppure in quell'ora specifica
            if a_item.get('Tipo') == "🔴 Tutto il Giorno" or ora_reg in a_item.get('Ore', []):
                is_supp_assente = True
                break
    if is_supp_assente:
        conflitti_supplenti_assenti.append({
            'Giorno': g_reg,
            'Ora': ora_reg,
            'Classe': reg.get('Classe'),
            'Docente Assente Originario': reg.get('Docente Assente'),
            'Supplente Assente': supp_reg
        })

if conflitti_supplenti_assenti:
    st.error(f"🚨 **ALLERTA CRITICA ({len(conflitti_supplenti_assenti)} CONFLITTO/I RILEVATO/I):** Ci sono supplenze assegnate a docenti che sono stati successivamente segnati come **ASSENTI**!")
    for conf in conflitti_supplenti_assenti:
        st.warning(f"⚠️ **{conf['Giorno']} - {conf['Ora']} (Classe {conf['Classe']}):** Il supplente assegnato **{conf['Supplente Assente']}** risulta **ASSENTE**! (Classe di {conf['Docente Assente Originario']} rimasta scoperta). Vai al TAB 2 per riassegnare.")
...
with tab_assenze:
    ...
    with col_add3:
        ...
        # Controllo preventivo se il docente selezionato era stato assegnato come supplente
        supplenze_gia_assegnate = [
            reg for reg in st.session_state.registro 
            if reg.get('Giorno') == giorno_selezionato and reg.get('Docente Sostituto') == doc_ass_sel
        ]
        if supplenze_gia_assegnate:
            det_supp = ", ".join([f"Classe {r.get('Classe')} ({r.get('Ora').split(' ')[0]})" for r in supplenze_gia_assegnate])
            st.error(f"⚠️ **ATTENZIONE:** Il docente **{doc_ass_sel}** era già stato assegnato come **SUPPLENTE** per {det_supp}! Inserendolo tra gli assenti, quelle supplenze diventeranno scoperte e dovranno essere riassegnate nel TAB 2.")
        ...
        if tipo_assenza == "🔴 Tutto il Giorno":
            ...
            if not ore_lezione_doc:
                ...
                msg_disp_note = f"\n\n*(Nota: {len(ore_disp_doc)} ora/e a disposizione non richiedono sostituto)*" if ore_disp_doc else ""
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Verifica se questo assente era anche un supplente assegnato
            era_supplente = any(
                reg.get('Giorno') == giorno_selezionato and reg.get('Docente Sostituto') == item['Docente']
                for reg in st.session_state.registro
            )
            badge_supp = " ⚠️ *(Era registrato come supplente!)*" if era_supplente else ""
            
            tc1.markdown(f"👤 **{item['Docente']}**{badge_supp}")
    ...
    # Tabella riassuntiva di TUTTE le assenze registrate durante la settimana
    st.markdown("---")
...
with tab_sostituzioni:
    ...
    # Calcolo dei docenti assenti per ciascuna ora specifica
    ...
    # Calcolo dei docenti già assegnati come sostituti per ciascuna ora
        ...
        for slot in item['Ore']:
            ...
            if cls_val and cls_val.upper() != 'NAN' and cls_val != '':
                if cls_val.upper() in ['DISP', 'D', 'DISPOSIZIONE']:
                    ...
                    # VERIFICA SE IL SOSTITUTO ASSEGNATO È RISULTATO ASSENTE
                    ...
                    classi_scoperte_lista.append({
                        ...
                        'Sostituto Raw': sostituto_assegnato,
                        'Is Conflitto': is_sostituto_assente
                    })
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        th1, th2, th3, th4, th5, th6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.5, 0.8])
        th1.markdown("**Ora**")
        th2.markdown("**Classe**")
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            tc1, tc2, tc3, tc4, tc5, tc6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.5, 0.8])
            ...
            if row_s['Is Conflitto']:
                tc5.markdown(f"🚨 **CONFLITTO:** `{row_s['Sostituto Raw']}` *(RISULTA ASSENTE!)*")
                if tc6.button("❌", key=f"btn_del_conflict_{idx_row}_{row_s['Ora']}", help="Cancella associazione in conflitto"):
        ...
        st.markdown("---")
        st.subheader("🔍 Assegnazione / Riassegnazione Sostituto")
        
        opzioni_scoperte = []
        for idx, row_s in enumerate(classi_scoperte_lista):
            if row_s['Is Conflitto']:
                stato_icon = "🚨 CONFLITTO (Supplente Assente!)"
            elif row_s['Sostituto Assegnato'] != "🔴 IN SOSPESO":
                stato_icon = f"🟢 {row_s['Sostituto Assegnato']}"
            else:
        ...
        is_conf = target_slot['Is Conflitto']
        
        if is_conf:
            st.error(f"🚨 **ALLERTA:** Il sostituto precedentemente assegnato (**{target_slot['Sostituto Raw']}**) è stato registrato come **ASSENTE** per {ora_target}! La classe **{classe_target}** richiede un NUOVO sostituto.")
        else:
        ...
        if all_cands:
            ...
            lbl_btn = "✅ Riassegna Nuovo Sostituto" if is_conf else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        reg_giorno_display = []
        for r in reg_giorno:
            # Verifica se il sostituto in questa riga risulta assente
            supp = r.get('Docente Sostituto')
            ora = r.get('Ora')
            g = r.get('Giorno')
            is_ass = False
            for a_item in st.session_state.assenze_per_giorno.get(g, []):
                if a_item.get('Docente') == supp:
                    if a_item.get('Tipo') == "🔴 Tutto il Giorno" or ora in a_item.get('Ore', []):
                        is_ass = True
                        break
            
            r_copy = dict(r)
            r_copy['Stato Sostituto'] = "🚨 ASSENTE (Da Riassegnare)" if is_ass else "🟢 Confermato"
            reg_giorno_display.append(r_copy)

        reg_giorno_sorted = sort_registro_by_day_and_teacher(reg_giorno_display)
        df_reg_giorno = pd.DataFrame(reg_giorno_sorted)
        
        # Riordina colonne per chiarezza
        cols_order = ['Giorno', 'Ora', 'Docente Assente', 'Classe', 'Materia', 'Docente Sostituto', 'Stato Sostituto', 'Tipologia Sostituzione', 'Priorità Applicata']
        existing_cols = [c for c in cols_order if c in df_reg_giorno.columns]
        df_reg_giorno = df_reg_giorno[existing_cols]
    ...
    if st.session_state.registro:
        ...
        for r in st.session_state.registro:
            ...
            r_copy = dict(r)
            r_copy['Stato Sostituto'] = "🚨 ASSENTE (Da Riassegnare)" if is_ass else "🟢 Confermato"
            reg_completo_display.append(r_copy)

        reg_completo_sorted = sort_registro_by_day_and_teacher(reg_completo_display)
        df_reg_completo = pd.DataFrame(reg_completo_sorted)
        
        cols_order = ['Giorno', 'Ora', 'Docente Assente', 'Classe', 'Materia', 'Docente Sostituto', 'Stato Sostituto', 'Tipologia Sostituzione', 'Priorità Applicata']
        existing_cols = [c for c in cols_order if c in df_reg_completo.columns]
        df_reg_completo = df_reg_completo[existing_cols]
