...
def save_state_to_file():
    try:
        data = {
            'assenze_per_giorno': st.session_state.get('assenze_per_giorno', {g: [] for g in GIORNI}),
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                if 'assenze_per_giorno' in data and isinstance(data['assenze_per_giorno'], dict):
                    for g in GIORNI:
                        if g not in data['assenze_per_giorno']:
                            data['assenze_per_giorno'][g] = []
                    st.session_state.assenze_per_giorno = data['assenze_per_giorno']
                if 'registro' in data and isinstance(data['registro'], list):
                    st.session_state.registro = data['registro']
        except Exception:
            pass

### Inizializzazione Persistenza e Session State
if 'loaded_persistent_data' not in st.session_state:
    st.session_state.loaded_persistent_data = True
...
st.sidebar.download_button(
    ...
    data=json_str_backup.encode('utf-8'),
    file_name="backup_sostituzioni_calvino.json",
    mime="application/json",
    use_container_width=True,
    help="Scarica un file di salvataggio con tutte le assenze e sostituzioni finora registrate"
)

uploaded_backup = st.sidebar.file_uploader("📂 Ripristina da file Backup (JSON)", type=['json'], key="uploader_backup_json")
if uploaded_backup is not None:
    try:
        backup_data = json.load(uploaded_backup)
        if 'assenze_per_giorno' in backup_data and 'registro' in backup_data:
            st.session_state.assenze_per_giorno = backup_data['assenze_per_giorno']
            st.session_state.registro = backup_data['registro']
            save_state_to_file()
            st.sidebar.success("✅ Sessione ripristinata dal file backup!")
            st.rerun()
        else:
            st.sidebar.error("Formato file JSON non valido.")
    except Exception as e:
        st.sidebar.error(f"Errore caricamento backup: {e}")

if st.sidebar.button("🗑️ Reset Totale Dati Sessione", use_container_width=True):
    st.session_state.assenze_per_giorno = {g: [] for g in GIORNI}
    st.session_state.registro = []
    if os.path.exists(DATA_FILE):
        try:
            ...
            pass
    st.sidebar.success("Tutti i dati sono stati azzerati!")
    st.rerun()
