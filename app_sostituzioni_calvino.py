...
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Multi-Assenze (A.S. 2026/2027)")
...
if 'assenze_giorno' not in st.session_state:
    st.session_state.assenze_giorno = [] # lista di dict: {'Docente': str, 'Tipo': 'Tutto il giorno'/'Permesso', 'Ore': [str]}
...
default_teachers_data = [
    ...
    {
        'Docente': 'DE TURSI', 'Materia': 'Scienze Naturali', 'Indirizzo': 'ITE',
        'CdC': '1a, 1b, 2b, 1c, 2c',
        'Lunedì_1ª ora (08:00)': '1c', 'Lunedì_2ª ora (08:55)': '1b', 'Lunedì_3ª ora (10:00)': '1b', 'Lunedì_4ª ora (10:55)': '2c', 'Lunedì_5ª ora (12:00)': '1c', 'Lunedì_6ª ora (12:55)': '1b', 'Lunedì_7ª ora (14:00)': '2c',
        'Martedì_1ª ora (08:00)': '1a', 'Martedì_2ª ora (08:55)': '2b', 'Martedì_3ª ora (10:00)': '1a', 'Martedì_4ª ora (10:55)': '1a', 'Martedì_5ª ora (12:00)': '2b', 'Martedì_6ª ora (12:55)': '2c', 'Martedì_7ª ora (14:00)': '2b'
    }
]
...
with tab_assenze:
    ...
    for slot in ORE:
        ...
        if val and val.upper() != 'NAN' and val != '':
            ore_lezione_doc.append((slot, val))
    ...
    with col_add3:
        if tipo_assenza == "🔴 Tutto il Giorno":
            ore_selezionate_ass = [slot for slot, val in ore_lezione_doc]
            st.info(f"Verranno inserite tutte le {len(ore_selezionate_ass)} ore in orario del docente per {giorno_selezionato}.")
        else:
            opzioni_ore_doc = [f"{slot} (Classe {val})" if val != 'DISP' else f"{slot} (DISP)" for slot, val in ore_lezione_doc]
            if not opzioni_ore_doc:
                st.warning("Nessuna lezione trovata a orario per questo docente.")
                ore_selezionate_ass = []
            else:
                sel_labels = st.multiselect("Seleziona le ore di Permesso / Assenza:", opzioni_ore_doc, default=opzioni_ore_doc)
                ore_selezionate_ass = [lbl.split(" (")[0] for lbl in sel_labels]
    ...
    if st.session_state.assenze_giorno:
        ...
        with c_del2:
            if st.button("🗑️ Svuota Elenco Assenti"):
                st.session_state.assenze_giorno = []
                st.rerun()
    else:
...
with tab_sostituzioni:
    ...
    for reg in st.session_state.registro:
        if reg.get('Giorno') == giorno_selezionato:
            docenti_sostituti_occupati_per_ora[reg.get('Ora')].add(reg.get('Docente Sostituto'))
    ...
    for item in st.session_state.assenze_giorno:
        ...
        for slot in item['Ore']:
            ...
            # Verifica se la sostituzione è già stata registrata nel Session State
    ...
    if not st.session_state.assenze_giorno:
        ...
        # Selezione della classe/ora specifica da risolvere
        opzioni_scoperte = []
        ...
        # Mostra i blocchi di sicurezza (esclusioni)
        esclusi_assenti = docenti_assenti_per_ora[ora_target]
        esclusi_sostituti = docenti_sostituti_occupati_per_ora[ora_target]
        ...
        # Ricerca Sostituti in Tempo Reale
        ...
        for idx, r in df_schedule.iterrows():
            ...
            # FILTRO DI SICUREZZA FONDAMENTALE: Esclude chiunque sia assente in quell'ora o già assegnato a supplenze
        ...
        if all_cands:
            ...
            if st.button("✅ Conferma Assegnazione Sostituto", type="primary"):
                # Rimuovi eventuale sostituzione precedente per lo stesso slot/assente
                st.session_state.registro = [
