...
# Iniezione CSS per personalizzazione font schede e selettore giorno
st.markdown("""
<style>
    /* 1. Stile per le tre pagine (Tab) per farle rientrare tutte e 3 affiancate nella schermata */
    ...
    /* 2. Selezione Giorno e Docenti: TESTO NON IN GRASSETTO (font-weight: normal) */
    
    /* Etichetta sopra il Selectbox */
    ...
    /* Testo SELEZIONATO nella casella del Selectbox (Giorno e Docente Assente NON in grassetto) */
    ...
    /* Opzioni dentro la tendina aperta (Popover Dropdown) - NON in grassetto */
    ...
    /* Spaziatura ordinata per le opzioni della tendina */
    li[data-baseweb="option"],
    [role="option"] {
    ...
    /* Evidenziazione opzione al passaggio del mouse o selezionata */
...
def create_circolare_pdf_bytes(giorno, data_str, num_circolare, rows, oggetto_str=""):
    ...
    # Titolo e Numero Circolare
    pdf.set_font('Helvetica', 'B', 13)
    pdf.set_text_color(26, 35, 126)
    pdf.cell(0, 6, 'CIRCOLARE GIORNALIERA SOSTITUZIONI E SUPPLENZE', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    
    pdf.set_font('Helvetica', 'B', 10)
    pdf.set_text_color(80, 80, 80)
    if num_circolare:
        pdf.cell(0, 5, clean_pdf_text(f"Circolare N. {num_circolare}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.cell(0, 5, clean_pdf_text(f"Disposizioni per la giornata di {giorno.upper()} - {data_str}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.ln(3)
    
    if oggetto_str:
        pdf.set_font('Helvetica', 'B', 9)
        pdf.set_text_color(26, 35, 126)
        pdf.cell(0, 5, clean_pdf_text(f"Oggetto: {oggetto_str}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='L')
        pdf.ln(2)

    pdf.set_font('Helvetica', '', 9)
    pdf.set_text_color(30, 30, 30)
    txt_intro = clean_pdf_text(f"Si comunicano alle SS.VV. le variazioni d'orario e le sostituzioni dei docenti assenti per la giornata di {giorno} ({data_str}). I docenti sostituti sono tenuti a firmare la presenza in classe sul Registro Elettronico.")
    pdf.multi_cell(0, 4.5, txt_intro)
    pdf.ln(4)
    
    if not rows:
        pdf.set_font('Helvetica', 'I', 10)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 8, "Nessuna sostituzione o variazione d'orario programmata per la giornata odierna.", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    else:
        # Table Header
        pdf.set_font('Helvetica', 'B', 8.5)
        pdf.set_fill_color(26, 35, 126) # #1a237e
        pdf.set_text_color(255, 255, 255)
        
        # Col widths: Ora (20), Classe (18), Assente (48), Materia (40), Sostituto (44), Note (20) -> total 190mm
        col_w = [20, 18, 48, 40, 44, 20]
        ...
        for r in rows:
            ...
            ora_str = clean_pdf_text(str(r.get('Ora', '')).split(' ')[0])
            cls_str = clean_pdf_text(str(r.get('Classe', '')))
            ass_str = clean_pdf_text(str(r.get('Docente Assente', '')))[:26]
            mat_str = clean_pdf_text(str(r.get('Materia', '')))[:22]
            sost_raw = str(r.get('Docente Sostituto', ''))
            sost_clean = clean_pdf_text(sost_raw)[:24]
            tipo_str = clean_pdf_text(str(r.get('Tipologia Sostituzione', '')))[:12]
            
            pdf.cell(col_w[0], 6.5, ora_str, border=1, align='C', fill=fill)
            pdf.cell(col_w[1], 6.5, cls_str, border=1, align='C', fill=fill)
            pdf.cell(col_w[2], 6.5, ass_str, border=1, align='L', fill=fill)
            pdf.cell(col_w[3], 6.5, mat_str, border=1, align='L', fill=fill)
            
            # Highlight if pending or conflict
            if 'SOSPESO' in sost_raw.upper() or '🔴' in sost_raw:
                pdf.set_text_color(180, 0, 0)
                pdf.set_font('Helvetica', 'B', 8)
            elif 'ASSENTE' in sost_raw.upper() or '🚨' in sost_raw:
            ...
            pdf.cell(col_w[4], 6.5, sost_clean, border=1, align='L', fill=fill)
            
            pdf.set_text_color(30, 30, 30)
            pdf.set_font('Helvetica', '', 8)
            pdf.cell(col_w[5], 6.5, tipo_str, border=1, align='C', fill=fill)
            pdf.ln()
            fill = not fill
    ...
    pdf.cell(90, 5, 'Il Dirigente Scolastico / Il Collaboratore del DS', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.cell(100, 5, '', new_x=XPos.RIGHT, new_y=YPos.TOP)
    pdf.set_font('Helvetica', 'I', 8)
    pdf.cell(90, 5, "(Firma autografa omessa ai sensi dell'art. 3 c. 2 d.lgs. n. 39/1993)", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')

    return bytes(pdf.output())

def create_circolare_text_formatted(giorno, data_str, num_circolare, rows, oggetto_str=""):
    num_str = f" N. {num_circolare}" if num_circolare else ""
    header_txt = "================================================================================\n"
    header_txt += "ISTITUTO DI ISTRUZIONE SUPERIORE \"ITALO CALVINO\" - ROZZANO (MI)\n"
    header_txt += "Via Guido Rossa - 20089 ROZZANO (MI) - Tel. 0257500115\n"
    header_txt += "Email: MIIS01900L@istruzione.it - Pec: MIIS01900L@pec.istruzione.it - Web: www.istitutocalvino.edu.it\n"
    header_txt += "================================================================================\n\n"
    header_txt += f"CIRCOLARE GIORNALIERA SOSTITUZIONI E SUPPLENZE{num_str}\n"
    header_txt += f"Disposizioni per la giornata di: {giorno.upper()} - {data_str}\n"
    
    if oggetto_str:
        header_txt += f"Oggetto: {oggetto_str}\n"

    header_txt += f"\nSi comunicano le variazioni d'orario e le sostituzioni dei docenti assenti per la giornata di {giorno} ({data_str}).\n"
    header_txt += "I docenti sostituti sono tenuti a firmare la presenza in classe sul Registro Elettronico.\n\n"
    header_txt += "--------------------------------------------------------------------------------\n"
    header_txt += "ELENCO SOSTITUZIONI E CLASSI INTERESSATE:\n"
    header_txt += "--------------------------------------------------------------------------------\n"

    if not rows:
        header_txt += "Nessuna sostituzione o variazione d'orario programmata per la giornata odierna.\n"
    else:
        for idx, r in enumerate(rows, 1):
            ora_full = str(r.get('Ora', ''))
            cls_str = str(r.get('Classe', ''))
            ass_str = str(r.get('Docente Assente', ''))
            mat_str = str(r.get('Materia', ''))
            sost_str = str(r.get('Docente Sostituto', ''))
            tipo_str = str(r.get('Tipologia Sostituzione', ''))
            prio_str = str(r.get('Priorità Applicata', ''))
            
            header_txt += f"{idx}. {ora_full.upper()} | CLASSE: {cls_str}\n"
            header_txt += f"   • Docente Assente:   {ass_str} ({mat_str})\n"
            header_txt += f"   • Docente Sostituto: {sost_str}\n"
            if tipo_str and tipo_str != '-':
                header_txt += f"   • Tipologia:         {tipo_str} ({prio_str})\n"
            header_txt += "--------------------------------------------------------------------------------\n"

    header_txt += "\nIl Dirigente Scolastico / Il Collaboratore del DS\n"
    header_txt += "I.I.S. \"Italo Calvino\" - Rozzano\n"
    return header_txt

def safe_gsheets_read(conn, worksheet_name):
    try:
        spr = conn._client.open_by_url(conn.url)
...
CLASSES_INFO = {
    '1A': ('LS', 'Liceo Scientifico'),
    '2A': ('LS', 'Liceo Scientifico'),
    '3A': ('LS', 'Liceo Scientifico'),
    '4A': ('LS', 'Liceo Scientifico'),
    '5A': ('LS', 'Liceo Scientifico'),
    '1B': ('LS', 'Liceo Scientifico'),
    '2B': ('LS', 'Liceo Scientifico'),
    '3B': ('LS', 'Liceo Scientifico'),
    '4B': ('LS', 'Liceo Scientifico'),
    '5B': ('LS', 'Liceo Scientifico'),
    '1D': ('LSU', 'Liceo Scienze Umane'),
    '2D': ('LSU', 'Liceo Scienze Umane'),
    '3D': ('LSU', 'Liceo Scienze Umane'),
    '4D': ('LSU', 'Liceo Scienze Umane'),
    '5D': ('LSU', 'Liceo Scienze Umane'),
    '1E': ('LSU', 'Liceo Scienze Umane'),
    '2E': ('LSU', 'Liceo Scienze Umane'),
    '3E': ('LSU', 'Liceo Scienze Umane'),
    '1F': ('LSU', 'Liceo Scienze Umane'),
    '2F': ('LSU', 'Liceo Scienze Umane'),
    '3F': ('LSU', 'Liceo Scienze Umane'),
    '4F': ('LSU', 'Liceo Scienze Umane'),
    '5F': ('LSU', 'Liceo Scienze Umane'),
    '1H': ('LSU', 'Liceo Scienze Umane'),
    '2H': ('LSU', 'Liceo Scienze Umane'),
    '3H': ('LSU', 'Liceo Scienze Umane'),
    '4H': ('LSU', 'Liceo Scienze Umane'),
    ...
    '1c': ('ITE', 'Istituto Tecnico Economico'),
    '2c': ('ITE', 'Istituto Tecnico Economico'),
    '3c': ('ITE', 'Istituto Tecnico Economico'),
    '4c': ('ITE', 'Istituto Tecnico Economico'),
    '5c': ('ITE', 'Istituto Tecnico Economico'),
    '3d': ('ITE', 'Istituto Tecnico Economico'),
    '3e': ('ITE', 'Istituto Tecnico Economico')
}
...
def import_registro_from_csv(uploaded_csv_file, replace_existing=True):
    """ Legge un file CSV esportato in precedenza dal Registro Sostituzioni e ripristina i dati nella sessione corrente. """
    try:
...
### ==============================================================================
### SIDEBAR: CARICAMENTI, IMPORTAZIONI E SALVATAGGIO CLOUD
### ==============================================================================
st.sidebar.title("⚙️ Pannello Controllo")

### 1. CARICAMENTO FILE ORARIO
st.sidebar.header("📂 1. Caricamento Orario")
uploaded_file = st.sidebar.file_uploader("Carica Orario Docenti (Excel / CSV)", type=['xlsx', 'csv'], key="sidebar_schedule_uploader")

default_teachers_data = [
    {
        'Docente': 'ACCIAVATTI Luciana',
        'Materia': 'Filosofia e Storia',
        'Indirizzo': 'Liceo Scientifico',
        'CdC': '3B, 4B, 5B, 4H',
        'Lunedì_1ª ora (08:00)': '5B',
        'Lunedì_2ª ora (08:55)': '4B',
        'Lunedì_3ª ora (10:00)': '3B',
        'Lunedì_4ª ora (10:55)': 'DISP',
        'Lunedì_5ª ora (12:00)': '5B',
        'Lunedì_6ª ora (12:55)': '4H',
        'Lunedì_7ª ora (14:00)': '4H',
        'Martedì_1ª ora (08:00)': '5B',
        'Martedì_2ª ora (08:55)': '4B',
        'Martedì_3ª ora (10:00)': '4B',
        'Martedì_4ª ora (10:55)': '3B',
        'Martedì_5ª ora (12:00)': '5B'
    },
    {
        'Docente': 'AGORINI',
        'Materia': 'Lab. Informatica (Compresenza)',
        'Indirizzo': 'ITE',
        'CdC': '3c, 4c, 5c',
        'Lunedì_5ª ora (12:00)': '3c',
        'Lunedì_6ª ora (12:55)': '3c',
        'Lunedì_7ª ora (14:00)': '4c'
    },
    {
        'Docente': 'COMINI Fabio',
        'Materia': 'Filosofia e Storia',
        'Indirizzo': 'Liceo Scientifico',
        'CdC': '4A, 5A, 3D, 4D',
        'Lunedì_1ª ora (08:00)': '4A',
        'Lunedì_2ª ora (08:55)': '4A',
        'Lunedì_3ª ora (10:00)': '4D',
        'Lunedì_4ª ora (10:55)': '5A',
        'Lunedì_5ª ora (12:00)': '5A',
        'Lunedì_6ª ora (12:55)': '5A'
    }
]
...
if uploaded_file is not None:
    try:
        ...
        st.sidebar.success(f"✅ Orario Caricato: {len(df_schedule)} docenti")
    except Exception as e:
...
### 2. SALVATAGGIO CLOUD GOOGLE FOGLI
st.sidebar.markdown("---")
...
### ==============================================================================
### SELETTORE DEL GIORNO PROMINENTE IN PRIMA PAGINA (PAGINA PRINCIPALE)
### ==============================================================================
st.markdown("---")
c_day1, c_day2 = st.columns([1.8, 2.2])
...
### Calcolo avanzamento coperture del GIORNO e della SETTIMANA
tot_scop_g = 0
in_sosp_g = 0
confl_g = 0
...
with c_day2:
    if tot_scop_g > 0:
        if in_sosp_g == 0 and confl_g == 0:
            st.success(f"🎉  **{giorno_selezionato.upper()}** : TUTTE LE CLASSI SONO COPERTE! ({tot_scop_g}/{tot_scop_g})")
        else:
            cop_g = tot_scop_g - in_sosp_g - confl_g
            st.warning(f"⏳  **{giorno_selezionato}** : {cop_g}/{tot_scop_g} coperte | 🔴 {in_sosp_g} in sospeso" + (f" | 🚨 {confl_g} in conflitto" if confl_g > 0 else ""))
    elif st.session_state.get('assenze_per_giorno', {}).get(giorno_selezionato):
        st.info(f"ℹ️  **{giorno_selezionato}** : Tutte le assenze riguardano ore a disposizione (nessuna classe da coprire).")
    else:
        st.info(f"ℹ️  **{giorno_selezionato}** : Nessun docente assente inserito al momento per oggi.")
...
with tab_assenze:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        with st.container(height=400):
            ...
            assenze_sorted = sorted(list(st.session_state.assenze_per_giorno[giorno_selezionato]), key=lambda x: str(x.get('Docente', '')).strip().upper())
            for idx, item in enumerate(assenze_sorted):
...
with tab_sostituzioni:
    ...
    # Ordinamento: Prima le lezioni in sospeso (🔴) o in conflitto (🚨), poi quelle già coperte (🟢)
...
with tab_registro:
    ...
    def get_full_sostituzioni_report(df_sched, giorno_target=None):
        giorni_to_process = [giorno_target] if (giorno_target and giorno_target in GIORNI) else GIORNI
        all_rows = []

        for g in giorni_to_process:
            processed_keys = set()

            if 'assenze_per_giorno' in st.session_state and g in st.session_state.assenze_per_giorno:
                for item_ass in st.session_state.assenze_per_giorno[g]:
                    d_nome = item_ass['Docente']
                    r_doc = df_sched[df_sched['Docente'] == d_nome]
                    r_info = r_doc.iloc[0] if len(r_doc) > 0 else {}
                    d_mat = str(r_info.get('Materia', item_ass.get('Materia', 'Generale')))
                    ore_eff = item_ass['Ore'] if item_ass.get('Tipo') != "🔴 Tutto il Giorno" else ORE
                    ...
                    for slot in ore_eff:
                        ...
                        if cls_val and cls_val.upper() not in ['', 'NAN', 'NONE', 'DISP', 'D', 'DISPOSIZIONE']:
                            key_tuple = (g, slot, d_nome)
                            processed_keys.add(key_tuple)

                            r_reg = None
                            for reg in st.session_state.get('registro', []):
                                if reg.get('Giorno') == g and reg.get('Ora') == slot and reg.get('Docente Assente') == d_nome:
                                    r_reg = reg
                                    break

                            if r_reg:
                                s_nome = str(r_reg.get('Docente Sostituto', '')).strip()
                                if s_nome and is_docente_assente_in_ora(s_nome, g, slot):
                                    stato = f"🚨 ASSENTE ({s_nome})"
                                elif not s_nome or "🔴" in s_nome or "SOSPESO" in s_nome.upper() or s_nome.upper() in ['NONE', 'NAN', '']:
                                    stato = "🔴 IN SOSPESO"
                                else:
                                    stato = "🟢 Presente"
                                ...
                                all_rows.append({
                                    ...
                                    'Classe': cls_val,
                                    'Materia': d_mat,
                                    'Docente Sostituto': s_nome if s_nome else '🔴 IN SOSPESO',
                                    'Tipologia Sostituzione': r_reg.get('Tipologia Sostituzione', 'EXTRA'),
                                    'Stato Sostituto': stato,
                                    'Priorità Applicata': r_reg.get('Priorità Applicata', '-')
                                })
                            else:
                                all_rows.append({
                                    'Giorno': g,
                                    ...
                                    'Classe': cls_val,
                                    'Materia': d_mat,
                                    'Docente Sostituto': '🔴 IN SOSPESO',
                                    'Tipologia Sostituzione': '-',
                                    'Stato Sostituto': '🔴 IN SOSPESO',
                                    'Priorità Applicata': 'Da Assegnare'
                                })

            for r_reg in st.session_state.get('registro', []):
                if r_reg.get('Giorno') == g:
                    d_ass = str(r_reg.get('Docente Assente', '')).strip()
                    slot = str(r_reg.get('Ora', '')).strip()
                    key_tuple = (g, slot, d_ass)
                    if key_tuple not in processed_keys:
                        ...
                        all_rows.append({
                            ...
                            'Ora': slot,
                            'Docente Assente': d_ass,
                            'Classe': r_reg.get('Classe', ''),
                            'Materia': r_reg.get('Materia', ''),
                            'Docente Sostituto': s_nome if s_nome else '🔴 IN SOSPESO',
                            'Tipologia Sostituzione': r_reg.get('Tipologia Sostituzione', '-'),
                            'Stato Sostituto': stato,
                            'Priorità Applicata': r_reg.get('Priorità Applicata', '-')
                        })
        ...
        def sort_key_base(r):
            ...
            o_val = str(r.get('Ora', ''))
            o_idx = 99
            for i, slot_name in enumerate(ORE):
                if slot_name == o_val or slot_name.split(' ')[0] in o_val or o_val.split(' ')[0] in slot_name:
                    o_idx = i
                    break
            return (g_idx, d_ass, o_idx, str(r.get('Classe', '')))

        def get_cat_rank(r):
            st_val = str(r.get('Stato Sostituto', ''))
            if "🚨" in st_val or "ASSENTE (" in st_val:
                return 0
            elif "🔴" in st_val or "SOSPESO" in st_val.upper():
                return 1
            else:
                return 2

        all_rows = sorted(all_rows, key=sort_key_base)
        all_rows = sorted(all_rows, key=get_cat_rank)
        return all_rows

    rows_giorno = get_full_sostituzioni_report(df_schedule, giorno_selezionato)

    if rows_giorno:
        df_reg_giorno = pd.DataFrame(rows_giorno)
        ...
        df_reg_giorno = df_reg_giorno[cols_order]
        ...
        st.info("Nessuna assenza o sostituzione registrata per la giornata di oggi.")

        st.markdown("---")
        st.subheader(f"📄 Generazione Circolare Giornaliera delle Supplenze ({giorno_selezionato})")
        st.write("Genera la **Circolare Ufficiale delle Supplenze** con carta intestata dell'Istituto, data, oggetto e riepilogo dettagliato delle sostituzioni.")

        col_circ1, col_circ2, col_circ3 = st.columns([1.5, 1.5, 3])
        with col_circ1:
            data_circ = st.date_input("Data Circolare", value=datetime.date.today(), key="data_circ_input")
            data_circ_str = data_circ.strftime("%d/%m/%Y")
        with col_circ2:
            num_circ = st.text_input("N° Circolare (opzionale)", value="", key="num_circ_input", placeholder="es. 124")
        with col_circ3:
            ogg_circ = st.text_input("Oggetto Circolare", value="Organizzazione supplenze e variazioni orario del giorno", key="ogg_circ_input")

        txt_circolare = create_circolare_text_formatted(
            giorno=giorno_selezionato,
            data_str=data_circ_str,
            num_circolare=num_circ,
            rows=rows_giorno,
            oggetto_str=ogg_circ
        )

        pdf_circolare_bytes = create_circolare_pdf_bytes(
            giorno=giorno_selezionato,
            data_str=data_circ_str,
            num_circolare=num_circ,
            rows=rows_giorno,
        ...
        with col_pdf:
            st.markdown("#### 🖨️ Formato PDF Ufficiale (Pronto per la Stampa)")
            st.caption("Contiene l'intestazione ufficiale dell'I.I.S. Italo Calvino, la tabella formattata delle sostituzioni e lo spazio per la firma del Dirigente Scolastico.")
            
            file_pdf_name = f"Circolare_Supplenze_{giorno_selezionato}_{data_circ_str.replace('/', '-')}.pdf"
            st.download_button(
                label="📥 Scarica Circolare PDF (Ufficiale con Carta Intestata)",
                data=pdf_circolare_bytes,
                file_name=file_pdf_name,
                mime="application/pdf",
                type="primary",
                key="btn_dl_circolare_pdf"
            )

        with col_txt:
            st.markdown("#### 📝 Formato Testo (Pronto per Registro Elettronico / Email)")
            st.caption("Seleziona e copia il testo sottostante per incollarlo su Axios, Nuvola, ClasseViva o inviarlo via e-mail ai docenti interessati.")
            st.text_area("Testo Formattato della Circolare:", value=txt_circolare, height=280, key="txt_area_circolare")


    st.markdown("---")
    st.subheader("📜 Storico Completo Sessione (Tutti i Giorni — Ordinato per Criticità, Stato e Docente)")
    rows_completo = get_full_sostituzioni_report(df_schedule, None)
    if rows_completo:
        df_reg_completo = pd.DataFrame(rows_completo)
        cols_order = ['Giorno', 'Ora', 'Docente Assente', 'Classe', 'Materia', 'Docente Sostituto', 'Tipologia Sostituzione', 'Stato Sostituto', 'Priorità Applicata']
        df_reg_completo = df_reg_completo[cols_order]
