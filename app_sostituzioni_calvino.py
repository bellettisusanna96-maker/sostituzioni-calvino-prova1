...
docente_assente = st.sidebar.selectbox("1️⃣ Seleziona Docente Assente", docente_list, index=0)
giorno = st.sidebar.selectbox("2️⃣ Seleziona Giorno della Settimana", GIORNI, index=0)
...
# Estrazione lezioni del docente assente per il giorno selezionato
lezioni_giorno = []
for slot in ORE:
    k = f"{giorno}_{slot}"
    val = str(row_doc.get(k, '')).strip()
    if val and val.upper() != 'NAN' and val != '':
        if val.upper() == 'DISP':
            lezioni_giorno.append((slot, 'DISP', 'Ora a Disposizione'))
        else:
            lezioni_giorno.append((slot, val, f"Classe {val}"))

st.sidebar.markdown("---")
st.sidebar.subheader(f"📅 Orario di {docente_assente} ({giorno})")

if lezioni_giorno:
    for slot, cls_val, desc in lezioni_giorno:
        if cls_val == 'DISP':
            st.sidebar.info(f"• **{slot}**: `{desc}`")
        else:
            st.sidebar.success(f"• **{slot}**: `{desc}`")
else:
    st.sidebar.warning(f"Nessuna lezione o ora DISP registrata il {giorno}.")

# Menu a tendina intelligente per la scelta dell'ora
opzioni_ore = []
for slot in ORE:
    k = f"{giorno}_{slot}"
    val = str(row_doc.get(k, '')).strip()
    if val and val.upper() != 'NAN' and val != '':
        if val.upper() == 'DISP':
            opzioni_ore.append(f"{slot} ➔ [DISP - Ora a Disposizione]")
        else:
            opzioni_ore.append(f"{slot} ➔ [Classe {val}]")
    else:
        opzioni_ore.append(f"{slot} ➔ [Libera / Nessuna lezione]")

# Indice predefinito: prima lezione utile trovata, altrimenti 0
def_idx = 0
for idx, (slot, cls_val, desc) in enumerate(lezioni_giorno):
    if cls_val != 'DISP':
        def_idx = ORE.index(slot)
        break

scelta_ora_str = st.sidebar.selectbox("3️⃣ Seleziona Ora da Sostituire", opzioni_ore, index=def_idx)
ora_selezionata = scelta_ora_str.split(" ➔ ")[0].strip()

slot_key = f"{giorno}_{ora_selezionata}"
...
if classe_rilevata and classe_rilevata.upper() != 'DISP' and classe_rilevata != 'nan' and classe_rilevata != '':
    ...
    st.sidebar.markdown("---")
    st.sidebar.warning("Classe non presente in orario per l'ora scelta.")
    classe_effettiva = st.sidebar.selectbox("Seleziona manualmente la classe:", list(CLASSES_INFO.keys()), index=9)
...
st.markdown(f"## 🔍 Ricerca Sostituto per **{docente_assente}**")
st.info(f"📅 **Giorno & Ora:** `{giorno}`, `{ora_selezionata}` | 📍 **Classe:** `{classe_effettiva}` ({indirizzo_nome}) | 📚 **Materia:** `{materia_docente}`")

# Identificazione docenti già assegnati ad altre sostituzioni nello STESSO giorno ed ora
docenti_gia_occupati_in_questo_slot = set()
for reg in st.session_state.registro:
    if reg.get('Giorno') == giorno and reg.get('Ora') == ora_selezionata:
        docenti_gia_occupati_in_questo_slot.add(reg.get('Docente Sostituto'))

if docenti_gia_occupati_in_questo_slot:
    st.warning(f"🔒 **Docenti già impegnati in altra supplenza il {giorno} alla {ora_selezionata}:** {', '.join(sorted(list(docenti_gia_occupati_in_questo_slot)))} (esclusi dai candidati per evitare sovrapposizioni).")
...
for idx, r in df_schedule.iterrows():
    ...
    # Escludi il docente assente e i docenti già assegnati a supplenze nello stesso slot
    if d_nome == docente_assente or d_nome in docenti_gia_occupati_in_questo_slot:
        continue
    ...
    if val_ora.upper() == 'DISP':
        ...
        ora_idx = ORE.index(ora_selezionata)
...
if all_candidates:
    ...
    if st.button("✅ Conferma e Registra Sostituzione", type="primary"):
        ...
        st.success(f"Sostituzione registrata con successo! **{docente_selezionato['Docente']}** coprirà la classe **{classe_effettiva}** ({giorno}, {ora_selezionata}).")
        st.rerun()
else:
    st.warning("Nessun docente disponibile o contattabile trovato per questa ora.")

st.markdown("---")
st.subheader("📊 Registro Annuale Sostituzioni Assegnate")
if st.session_state.registro:
    df_reg = pd.DataFrame(st.session_state.registro)
    st.dataframe(df_reg, use_container_width=True)
    
    if st.button("🗑️ Cancella Registro Sostituzioni"):
        st.session_state.registro = []
        st.rerun()
else:
