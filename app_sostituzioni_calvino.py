...
<style>
    ...
    /* Style for Streamlit Selectbox input field (light blue background) */
    ...
    /* Style for Dropdown Menu Popover (light blue background) */
    ...
    /* Hover & Selected Item: Dark Blue with White Text */
...
class CircolarePDF(FPDF):
    def header(self):
        ...
        self.cell(0, 8, 'I.I.S. ITALO CALVINO - ROZZANO', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
        self.set_font('Helvetica', 'B', 12)
        self.cell(0, 6, 'CIRCOLARE GIORNALIERA SOSTITUZIONI DOCENTI', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
        self.ln(4)
...
def create_circolare_pdf_bytes(giorno_sel, data_str, note_str, reg_giorno):
    pdf = CircolarePDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    pdf.cell(0, 6, f'Giorno: {clean_pdf_text(giorno_sel)} - Data: {clean_pdf_text(data_str)}', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    if note_str:
        pdf.cell(0, 6, f'Note: {clean_pdf_text(note_str)}', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(4)

    col_w = [18, 18, 42, 38, 48, 26]
    headers = ['Ora', 'Classe', 'Docente Assente', 'Materia', 'Docente Sostituto', 'Stato']
    ...
    if not reg_giorno:
        pdf.cell(sum(col_w), 8, 'Nessuna sostituzione registrata per questo giorno.', border=1, align='C')
        pdf.ln()
    else:
        for r in reg_giorno:
            ora_val = clean_pdf_text(r.get('Ora', '')).split(' ')[0]
            cls_val = clean_pdf_text(r.get('Classe', ''))
            ass_val = clean_pdf_text(r.get('Docente Assente', ''))
            mat_val = clean_pdf_text(r.get('Materia', ''))
            sost_val = clean_pdf_text(r.get('Docente Sostituto', ''))
            
            if sost_val and sost_val != '-':
                st_val = 'ASSEGNATO'
            else:
                st_val = 'IN SOSPESO'

            pdf.cell(col_w[0], 6, ora_val[:10], border=1, align='C')
            pdf.cell(col_w[1], 6, cls_val[:10], border=1, align='C')
            pdf.cell(col_w[2], 6, ass_val[:22], border=1, align='L')
            pdf.cell(col_w[3], 6, mat_val[:20], border=1, align='L')
            pdf.cell(col_w[4], 6, sost_val[:25], border=1, align='L')
            pdf.cell(col_w[5], 6, st_val[:12], border=1, align='C')
            pdf.ln()

    return bytes(pdf.output())

def create_storico_pdf_bytes(data_str, reg_tot):
    pdf = CircolarePDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)
    ...
    pdf.set_font('Helvetica', '', 10)
    pdf.cell(0, 6, f'Report Storico Completo Sostituzioni - Generato il: {clean_pdf_text(data_str)}', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(4)

    col_w = [22, 16, 16, 38, 32, 42, 24]
    headers = ['Giorno', 'Ora', 'Classe', 'Docente Assente', 'Materia', 'Docente Sostituto', 'Stato']
    ...
    pdf.set_font('Helvetica', '', 8)
    if not reg_tot:
        pdf.cell(sum(col_w), 8, 'Nessun dato nello storico.', border=1, align='C')
        pdf.ln()
    else:
        for r in reg_tot:
            ...
            pdf.cell(col_w[1], 6, ora_val[:8], border=1, align='C')
            pdf.cell(col_w[2], 6, cls_val[:8], border=1, align='C')
            pdf.cell(col_w[3], 6, ass_val[:20], border=1, align='L')
            pdf.cell(col_w[4], 6, mat_val[:16], border=1, align='L')
            pdf.cell(col_w[5], 6, sost_val[:22], border=1, align='L')
            pdf.cell(col_w[6], 6, st_val[:14], border=1, align='C')
            pdf.ln()
