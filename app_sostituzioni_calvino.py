...
def safe_gsheets_read(conn, worksheet_name):
    try:
        ...
        if client is not None and hasattr(conn, 'url') and conn.url:
            ...
            ws = spr.worksheet(worksheet_name)
...
def load_data_from_gsheets():
    ...
    try:
        ...
        if df_ora is not None and not df_ora.empty and 'Docente' in df_ora.columns:
            st.session_state.df_schedule = df_ora
...
def load_and_normalize_schedule(uploaded_file):
    ...
    if file_name.endswith('.pdf'):
        ...
        # 1. Prova con pdfplumber (se installata)
        try:
            import pdfplumber
        ...
        # 2. Prova con pypdf (fallback nativo)
        if not tables_rows:
            try:
                import pypdf
        ...
        # 3. Prova con PyPDF2 (legacy fallback)
        if not tables_rows:
            try:
                import PyPDF2
        ...
        df = pd.DataFrame(tables_rows[1:], columns=tables_rows[0])
    ...
    if 'Docente' in df.columns:
        df = df[df['Docente'].notna() & (df['Docente'].astype(str).str.strip() != '') & (~df['Docente'].astype(str).str.upper().str.contains('DOCENTE', na=False))]
