...
def is_docente_assente_in_ora(docente, giorno, ora):
    if not docente or docente == '🔴 IN SOSPESO':
        return False
    for item in st.session_state.assenze_per_giorno.get(giorno, []):
        if item['Docente'] == docente:
            if item.get('Tipo') == "🔴 Tutto il Giorno" or ora in item.get('Ore', []):
                return True
    return False

def get_conflitti_sostituzioni(giorno=None):
    conflitti = []
    target_giorni = [giorno] if giorno else GIORNI
    for g in target_giorni:
        for reg in st.session_state.registro:
            if reg.get('Giorno') == g:
                sost = reg.get('Docente Sostituto')
                ora = reg.get('Ora')
                if sost and sost != '🔴 IN SOSPESO' and is_docente_assente_in_ora(sost, g, ora):
                    conflitti.append({
                        'Giorno': g,
                        'Ora': ora,
...
tot_assenti_sett = sum(len(v) for v in st.session_state.assenze_per_giorno.values())
tot_reg_sett = len(st.session_state.registro)
tot_conf_sett = len(get_conflitti_sostituzioni())

st.sidebar.info(f"📊 **Stato In Memoria:**\n• Assenze inserite: `{tot_assenti_sett}`\n• Sostituzioni a registro: `{tot_reg_sett}`" + (f"\n• 🚨 Conflitti attivi: `{tot_conf_sett}`" if tot_conf_sett > 0 else ""))
...
# Banner di Allerta Globale per Conflitti
conflitti_giorno = get_conflitti_sostituzioni(giorno_selezionato)

if conflitti_giorno:
    st.error(f"🚨 **ALLERTA CRITICA SOSTITUZIONI DA RIPRENDERE ({len(conflitti_giorno)} CONFLITTO/I per {giorno_selezionato}):**")
    for c in conflitti_giorno:
        st.write(f"👉 **{c['Giorno']} - {c['Ora'].split(' ')[0]}** (Classe **{c['Classe']}**): Il sostituto **{c['Supplente Assente']}** risulta **ASSENTE**! La classe è rimasta scoperta e la sostituzione va modificata nel TAB 2.")
...
with tab_assenze:
    ...
    # Controllo se doc_ass_sel è già assegnato come supplente
    ...
    if supplenze_gia_assegnate:
        dett_supp = ", ".join([f"Classe {r.get('Classe')} ({r.get('Ora').split(' ')[0]})" for r in supplenze_gia_assegnate])
        st.error(f"🚨 **AVVISO CONFLITTO:** Il docente **{doc_ass_sel}** è attualmente assegnato come **SUPPLENTE** il {giorno_selezionato} per: **{dett_supp}**. Registrandolo come assente, tali supplenze risulteranno scoperte e dovranno essere riassegnate nel TAB 2!")
    ...
    with col_add3:
        ...
        if tipo_assenza == "🔴 Tutto il Giorno":
            ore_selezionate_ass = list(ORE) # L'assenza Tutto il giorno copre TUTTE e 7 le ore!
            if not ore_lezione_doc:
                st.warning(f"Nessuna lezione propria trovata in orario per **{doc_ass_sel}** il **{giorno_selezionato}** (sarà assente per l'intera giornata).")
            elif not ore_classe_doc:
                st.info(f"Il docente **{doc_ass_sel}** il **{giorno_selezionato}** non ha sue classi da sostituire" + (f" (ha {len(ore_disp_doc)} ora/e a disposizione)." if ore_disp_doc else "."))
            else:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            was_suppl = any(r.get('Docente Sostituto') == item['Docente'] for r in st.session_state.registro if r.get('Giorno') == giorno_selezionato)
            note_suppl = " ⚠️ *(Supplente)*" if was_suppl else ""
            
            tc1.markdown(f"👤 **{item['Docente']}**{note_suppl}")
            ...
            if item.get('Tipo') == "🔴 Tutto il Giorno":
                ore_str = (", ".join([o.split(" ")[0] for o in ore_classe_conf]) + " *(Tutto il giorno)*") if ore_classe_conf else "Tutto il giorno (nessuna classe propria)"
            else:
                ore_str = ", ".join([o.split(" ")[0] for o in ore_classe_conf]) if ore_classe_conf else "Nessuna classe propria"
    ...
    for g in GIORNI:
        for item in st.session_state.assenze_per_giorno.get(g, []):
            ...
            if item.get('Tipo') == "🔴 Tutto il Giorno":
                ore_str = (", ".join([o.split(" ")[0] for o in ore_classe_sett]) + " (Tutto il Giorno)") if ore_classe_sett else "Tutto il Giorno"
            else:
                ore_str = ", ".join([o.split(" ")[0] for o in ore_classe_sett]) if ore_classe_sett else "Nessuna classe propria"
...
with tab_sostituzioni:
    ...
    # Calcolo di TUTTI i docenti assenti per ciascuna ora
    ...
    # Calcolo di TUTTI i docenti occupati come sostituti (purché NON assenti)
    ...
    for reg in st.session_state.registro:
        if reg.get('Giorno') == giorno_selezionato:
            ...
            if s_doc and s_doc != '🔴 IN SOSPESO' and not is_docente_assente_in_ora(s_doc, giorno_selezionato, s_ora):
    ...
    for item in st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # Le ore di lezione propria da sostituire
        for slot in ORE:
            slot_key = f"{giorno_selezionato}_{slot}"
            cls_val = str(r_info.get(slot_key, '')).strip()
            
            # Verifichiamo se l'assenza copre questa ora
            if not is_docente_assente_in_ora(d_nome, giorno_selezionato, slot):
                continue
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            ...
            is_conf = row_s.get('Is Conflitto', False)
            ...
            if is_conf:
                ...
                if tc6.button("❌", key=f"btn_del_inline_{idx_row}_{row_s['Ora']}", help=f"Rimuovi sostituto assente {sost_val}"):
                    ...
                    save_persistent_data()
                    st.success(f"Rimosso sostituto in conflitto **{sost_val}**!")
                    st.rerun()
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            ...
            if row_s.get('Is Conflitto'):
                stato_icon = "🚨 CONFLITTO (Supplente Assente!)"
            elif s_val != "🔴 IN SOSPESO":
                stato_icon = "🟢 OK"
            else:
                stato_icon = "🔴 IN SOSPESO"
            opzioni_scoperte.append(f"{stato_icon} | {row_s['Ora']} | Assente: {row_s['Docente Assente']} | Classe: {row_s['Classe / Slot']} | Sostituto: {s_val}")
        ...
        is_target_conflitto = target_slot.get('Is Conflitto', False)
        
        if is_target_conflitto:
            st.error(f"🚨 **CONFLITTO RILEVATO:** Il sostituto **{sostituto_corrente}** è **ASSENTE** il {giorno_selezionato} in **{ora_target}**! La classe **{classe_target}** è nuovamente scoperta. Seleziona un nuovo sostituto in basso e clicca su 'Riassegna Nuovo Sostituto'.")
        else:
        ...
        if all_cands:
            ...
            btn_title = "✅ Confirm / Riassegna Nuovo Sostituto" if is_target_conflitto or sostituto_corrente != "🔴 IN SOSPESO" else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        reg_giorno_processed = []
        for r in reg_giorno:
            r_copy = dict(r)
            if is_docente_assente_in_ora(r.get('Docente Sostituto'), r.get('Giorno'), r.get('Ora')):
                r_copy['Stato Sostituto'] = f"🚨 ASSENTE ({r.get('Docente Sostituto')})"
            else:
                r_copy['Stato Sostituto'] = "🟢 Presente e Confermato"
            reg_giorno_processed.append(r_copy)
            
        reg_giorno_sorted = sort_registro_by_day_and_teacher(reg_giorno_processed)
    ...
    if st.session_state.registro:
        ...
        for r in st.session_state.registro:
            ...
            if is_docente_assente_in_ora(r.get('Docente Sostituto'), r.get('Giorno'), r.get('Ora')):
                ...
                r_copy['Stato Sostituto'] = "🟢 Presente e Confermato"
            reg_comp_processed.append(r_copy)
            
        reg_completo_sorted = sort_registro_by_day_and_teacher(reg_comp_processed)
