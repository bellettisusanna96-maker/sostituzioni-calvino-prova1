...
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Multi-Assenze con Cancellazione Singola (A.S. 2026/2027 - v14)")
...
with tab_sostituzioni:
    ...
    if not st.session_state.assenze_giorno:
        ...
        # Sezione Annullamento Rapido Sostituzione Già Assegnata
        sostituzioni_effettuate = [r for r in classi_scoperte_lista if r['Sostituto Assegnato'] != "🔴 IN SOSPESO"]
        if sostituzioni_effettuate:
            st.markdown("##### 🗑️ Annulla una Sostituzione Già Assegnata")
            col_canc1, col_canc2 = st.columns([3, 1])
            with col_canc1:
                sost_canc_idx = st.selectbox(
                    "Seleziona la sostituzione da annullare:",
                    range(len(sostituzioni_effettuate)),
                    format_func=lambda i: f"🟢 {sostituzioni_effettuate[i]['Ora']} | Classe {sostituzioni_effettuate[i]['Classe / Slot']} | Assente: {sostituzioni_effettuate[i]['Docente Assente']} ➔ Sostituto: {sostituzioni_effettuate[i]['Sostituto Assegnato']}",
                    key="sel_canc_sost_quick"
                )
            with col_canc2:
                if st.button("❌ Rimuovi Sostituto", key="btn_canc_sost_quick"):
                    target_canc = sostituzioni_effettuate[sost_canc_idx]
                    st.session_state.registro = [
                        reg for reg in st.session_state.registro 
                        if not (reg.get('Giorno') == giorno_selezionato and reg.get('Ora') == target_canc['Ora'] and reg.get('Docente Assente') == target_canc['Docente Assente'])
                    ]
                    st.success(f"Rimosso sostituto **{target_canc['Sostituto Assegnato']}** dalla lezione di {target_canc['Docente Assente']} ({target_canc['Classe / Slot']}, {target_canc['Ora']})!")
                    st.rerun()
        ...
        for idx, r in df_schedule.iterrows():
            ...
            # Se ha lezione con una classe in quell'ora, non è disponibile
            if val_ora and val_ora.upper() != 'DISP' and val_ora != 'nan' and val_ora in CLASSES_INFO:
                continue
            
            has_disp = (val_ora.upper() == 'DISP')
