...
with tab_sostituzioni:
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        if all_cands:
            ...
            st.info(f"👉 **Candidato Selezionato dalla Tabella:** **{cand_scelto['Docente']}** | Priorità: *{cand_scelto['Priorità']}* | Stato: *{cand_scelto['Stato']}*")
...
with tab_registro:
    ...
    def get_cat_rank(r):
        s_nome = str(r.get('Docente Sostituto', '')).strip()
        g_val = r.get('Giorno', '')
        o_val = r.get('Ora', '')
        if "🚨" in s_nome or "CONFLITTO" in s_nome or (s_nome and is_docente_assente_in_ora(s_nome, g_val, o_val)):
            return 0  # 🚨 Criticità
        elif not s_nome or "🔴" in s_nome or "SOSPESO" in s_nome.upper() or s_nome.upper() in ['NONE', 'NAN', '']:
            return 1  # 🔴 Pallini rossi (In sospeso)
        else:
            return 2  # 🟢 Pallini verdi (Presente / Assegnato)
    ...
    if reg_giorno:
        ...
        # Ordina mantenendo l'ordine attuale interno: prima criticità (0), poi pallini rossi (1), poi pallini verdi (2)
        reg_giorno_sorted = sorted(reg_giorno_sorted, key=get_cat_rank)
        ...
        for r in reg_giorno_sorted:
            ...
            if "🚨" in s_nome or "CONFLITTO" in s_nome or (s_nome and is_docente_assente_in_ora(s_nome, g_val, o_val)):
                ...
                stati_sost.append("🔴 IN SOSPESO")
            else:
                stati_sost.append("🟢 Presente")
        df_reg_giorno['Stato Sostituto'] = stati_sost
        if 'Priorità Applicata' in df_reg_giorno.columns and 'Stato Sostituto' in df_reg_giorno.columns:
            cols = list(df_reg_giorno.columns)
            cols.remove('Stato Sostituto')
            prio_idx = cols.index('Priorità Applicata')
            cols.insert(prio_idx, 'Stato Sostituto')
            df_reg_giorno = df_reg_giorno[cols]
    ...
    if st.session_state.registro:
        ...
        # Ordina mantenendo l'ordine attuale interno: prima criticità (0), poi pallini rossi (1), poi pallini verdi (2)
        reg_completo_sorted = sorted(reg_completo_sorted, key=get_cat_rank)
        ...
        for r in reg_completo_sorted:
            ...
            if "🚨" in s_nome or "CONFLITTO" in s_nome or (s_nome and is_docente_assente_in_ora(s_nome, g_val, o_val)):
                ...
                stati_sost_comp.append("🔴 IN SOSPESO")
            else:
                stati_sost_comp.append("🟢 Presente")
        df_reg_completo['Stato Sostituto'] = stati_sost_comp
        if 'Priorità Applicata' in df_reg_completo.columns and 'Stato Sostituto' in df_reg_completo.columns:
            cols = list(df_reg_completo.columns)
            cols.remove('Stato Sostituto')
            prio_idx = cols.index('Priorità Applicata')
            cols.insert(prio_idx, 'Stato Sostituto')
            df_reg_completo = df_reg_completo[cols]
