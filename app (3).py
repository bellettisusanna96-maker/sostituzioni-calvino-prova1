...
DATA_FILE = "dati_sostituzioni.json"
...
def save_persistent_data():
    data = {
        "assenze_per_giorno": st.session_state.get("assenze_per_giorno", {}),
...
# Inizializzazione Session State con file di persistenza
p_data = load_persistent_data()
if 'assenze_per_giorno' not in st.session_state:
    st.session_state.assenze_per_giorno = p_data.get("assenze_per_giorno", {g: [] for g in ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì']})
if 'registro' not in st.session_state:
    st.session_state.registro = p_data.get("registro", [])

st.title("🏫 I.I.S. 'Italo Calvino' - Rozzano")
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Gestione Settimanale e Storico Assenze (A.S. 2026/2027 - v23)")
...
def get_supplenti_assenti_conflicts():
    """Rileva se un docente assegnato come sostituto nel registro è stato successivamente segnato come assente."""
    conflicts = []
    for reg in st.session_state.registro:
        g = reg.get('Giorno')
        s_ora = reg.get('Ora')
        supplente = reg.get('Docente Sostituto')
        
        if not g or not supplente or supplente == "🔴 IN SOSPESO":
            continue
            
        assenti_g = st.session_state.assenze_per_giorno.get(g, [])
        for ass_item in assenti_g:
            if ass_item.get('Docente') == supplente:
                if s_ora in ass_item.get('Ore', []):
                    conflicts.append({
                        'Giorno': g,
                        'Ora': s_ora,
                        'Classe': reg.get('Classe'),
                        'Docente Assente Iniziale': reg.get('Docente Assente'),
                        'Supplente Assente': supplente
                    })
    return conflicts
...
default_teachers_data = [
    {
        ...
        'Lunedì_1ª ora (08:00)': '5B', 'Lunedì_2ª ora (08:55)': '4B', 'Lunedì_3ª ora (10:00)': '3B', 'Lunedì_4ª ora (10:55)': 'DISP',
        'Lunedì_5ª ora (12:00)': '5B', 'Lunedì_6ª ora (12:55)': '4H', 'Lunedì_7ª ora (14:00)': '4H',
    ...
    },
    {
        ...
        'Lunedì_1ª ora (08:00)': '4A', 'Lunedì_2ª ora (08:55)': '4A', 'Lunedì_3ª ora (10:00)': '4D', 'Lunedì_4ª ora (10:55)': '5A',
        'Lunedì_5ª ora (12:00)': '5A', 'Lunedì_6ª ora (12:55)': '5A'
    }
]
...
st.sidebar.markdown("---")
st.sidebar.header("💾 Persistenza & Backup Sessione")

# Riquadro di controllo dati in memoria
tot_assenti_count = sum(len(v) for v in st.session_state.assenze_per_giorno.values())
tot_registro_count = len(st.session_state.registro)
st.sidebar.info(f"📊 **Stato In Memoria:**\\n• {tot_assenti_count} docente/i assente/i\\n• {tot_registro_count} supplenza/e assegnata/e")

data_to_download = {
    "assenze_per_giorno": st.session_state.assenze_per_giorno,
    "registro": st.session_state.registro
}
json_str = json.dumps(data_to_download, ensure_ascii=False, indent=2)
st.sidebar.download_button(
    label="📥 Scarica Backup Sessione (JSON)",
    data=json_str,
    file_name="backup_sostituzioni_Calvino.json",
    mime="application/json"
)

uploaded_backup = st.sidebar.file_uploader("Ripristina da file Backup (JSON)", type=["json"])
if uploaded_backup is not None:
    try:
        b_data = json.load(uploaded_backup)
        if "assenze_per_giorno" in b_data:
            st.session_state.assenze_per_giorno = b_data["assenze_per_giorno"]
        if "registro" in b_data:
            st.session_state.registro = b_data["registro"]
        save_persistent_data()
        st.sidebar.success("✅ Sessione ripristinata dal file backup!")
        st.rerun()
    except Exception as e:
        st.sidebar.error(f"Errore ripristino: {e}")

if st.sidebar.button("🗑️ Reset Totale Dati Sessione"):
    st.session_state.assenze_per_giorno = {g: [] for g in GIORNI}
    st.session_state.registro = []
    save_persistent_data()
    st.rerun()

# Rilevamento globale dei conflitti (supplenti che risultano ora assenti)
all_conflicts = get_supplenti_assenti_conflicts()
if all_conflicts:
    st.error(
        f"🚨 **ATTENZIONE ALLERTA SOSTITUZIONI**: Rilevati **{len(all_conflicts)} conflitti**! "
        f"Uno o più docenti precedentemente assegnati come **supplenti** sono stati successivamente registrati come **ASSENTI**. "
        f"Vai nel TAB 2 per riassegnare le supplenze scoperte!"
    )
...
with tab_assenze:
    ...
    with col_add3:
        ...
        if tipo_assenza == "🔴 Tutto il Giorno":
            ...
            if not ore_lezione_doc:
                st.warning(f"Nessuna lezione trovata in orario per **{doc_ass_sel}** il **{giorno_selezionato}**.")
            elif not ore_classe_doc:
                st.info(f"Il docente **{doc_ass_sel}** il **{giorno_selezionato}** non ha classi da sostituire" + (f" (ha {len(ore_disp_doc)} ora/e a disposizione)." if ore_disp_doc else "."))
            else:
    ...
    if st.button("➕ Aggiungi Docente all'Elenco Assenti del Giorno", type="primary"):
        ...
        if gia_presente:
            ...
            # Controlla se il docente appena aggiunto era già stato assegnato come sostituto per qualcun altro
            supplenze_impattate = [
                r for r in st.session_state.registro
                if r.get('Giorno') == giorno_selezionato and r.get('Docente Sostituto') == doc_ass_sel and r.get('Ora') in ore_selezionate_ass
            ]
            if supplenze_impattate:
                st.warning(f"⚠️ **ATTENZIONE**: Il docente **{doc_ass_sel}** era stato assegnato come SUPPLENTE per {len(supplenze_impattate)} classe/i ({', '.join([r['Classe'] for r in supplenze_impattate])}). Quelle supplenze risultano ora SCOPERTE e vanno riassegnate nel TAB 2!")
            else:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Verifichiamo se l'assente era assegnato come sostituto
            era_sostituto = any(
                r.get('Docente Sostituto') == item['Docente'] and r.get('Giorno') == giorno_selezionato and r.get('Ora') in item['Ore']
                for r in st.session_state.registro
            )
            badge_sost = " ⚠️ *(Era registrato come supplente!)*" if era_sostituto else ""
            
            tc1.markdown(f"👤 **{item['Docente']}**{badge_sost}")
...
with tab_sostituzioni:
    ...
    # Controlliamo i conflitti specifici per il giorno selezionato
    conflicti_giorno = [c for c in all_conflicts if c['Giorno'] == giorno_selezionato]
    if conflicti_giorno:
        st.error(
            f"🚨 **ALERTA SUPPLENTI ASSENTI ({giorno_selezionato})**:\\n" +
            "\\n".join([
                f"• La classe **{c['Classe']}** ({c['Ora']}) aveva come sostituto **{c['Supplente Assente']}**, "
                f"ma questo docente è stato successivamente segnato come **ASSENTE**! Occorre assegnare un nuovo sostituto."
                for c in conflicti_giorno
            ])
        )
    ...
    for reg in st.session_state.registro:
        if reg.get('Giorno') == giorno_selezionato:
            s_ora = reg.get('Ora')
            s_doc = reg.get('Docente Sostituto')
            # Considera occupati solo i sostituti che NON sono a loro volta assenti in quell'ora
            if s_doc and s_doc not in docenti_assenti_per_ora.get(s_ora, set()):
                if s_ora in docenti_sostituti_occupati_per_ora:
                    docenti_sostituti_occupati_per_ora[s_ora].add(s_doc)
                else:
                    docenti_sostituti_occupati_per_ora[s_ora] = {s_doc}
        ...
        for slot in item['Ore']:
            ...
            if cls_val and cls_val.upper() != 'NAN' and cls_val != '':
                if cls_val.upper() in ['DISP', 'D', 'DISPOSIZIONE']:
                    ...
                    # Verifica se il sostituto assegnato è ora assente
                    is_sostituto_assente = False
                    if sostituto_assegnato and sostituto_assegnato in docenti_assenti_per_ora.get(slot, set()):
                        is_sostituto_assente = True
                    ...
                    classi_scoperte_lista.append({
                        ...
                        'Sostituto Assente Flag': is_sostituto_assente
                    })
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        th1, th2, th3, th4, th5, th6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.4, 1.0])
        th1.markdown("**Ora**")
        th2.markdown("**Classe**")
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            tc1, tc2, tc3, tc4, tc5, tc6 = st.columns([1.2, 0.8, 2.0, 2.0, 2.4, 1.0])
            ...
            if row_s.get('Sostituto Assente Flag'):
                tc5.markdown(f"🚨 **CONFLITTO**: ~~{sost_val}~~ *(ASSENTE!)*")
                if tc6.button("❌", key=f"btn_del_inline_{idx_row}_{row_s['Ora']}", help=f"Rimuovi sostituto assente {sost_val}"):
                    ...
                    save_persistent_data()
                    st.success(f"Rimosso sostituto assente **{sost_val}** per la classe {row_s['Classe / Slot']}!")
                    st.rerun()
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            if row_s.get('Sostituto Assente Flag'):
                stato_icon = "🚨 DA RIASSEGNARE (Supplente Assente!)"
            elif row_s['Sostituto Assegnato'] != "🔴 IN SOSPESO":
                stato_icon = f"🟢 Sostituto: {row_s['Sostituto Assegnato']}"
            else:
                stato_icon = "🔴 IN SOSPESO"
            opzioni_scoperte.append(f"{row_s['Ora']} | Classe: {row_s['Classe / Slot']} | Assente: {row_s['Docente Assente']} | [{stato_icon}]")
            
        idx_scoperta_sel = st.selectbox("Seleziona la lezione da coprire o riassegnare:", range(len(opzioni_scoperte)), format_func=lambda i: opzioni_scoperte[i])
        ...
        is_sost_assente = target_slot.get('Sostituto Assente Flag', False)
        
        if is_sost_assente:
            st.error(f"🚨 **ATTENZIONE**: Il sostituto precedentemente assegnato (**{sostituto_corrente}**) è risultato **ASSENTE**! Seleziona un nuovo sostituto dalla lista qui sotto per coprire la classe **{classe_target}** ({ora_target}).")
        else:
            st.info(f"📍 **Dettagli Lezione:** `{giorno_selezionato}`, `{ora_target}` | **Docente Assente:** `{docente_assente_target}` | **Classe:** `{classe_target}` | **Materia:** `{materia_target}`" + (f" | 🟢 **Sostituto Attuale:** `{sostituto_corrente}`" if sostituto_corrente != "🔴 IN SOSPESO" else ""))
        ...
        if all_cands:
            ...
            scelta_cand_idx = st.selectbox("Scegli il NUOVO docente da assegnare a questa classe:", range(len(labels_cands)), format_func=lambda i: labels_cands[i])
            
            cand_scelto = all_cands[scelta_cand_idx]
            
            btn_label = "✅ Riassegna Nuovo Sostituto" if is_sost_assente else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        # Aggiungiamo indicazione visiva se il docente sostituto è ora assente
        for row in reg_giorno_sorted:
            s_doc = row.get('Docente Sostituto')
            s_ora = row.get('Ora')
            assenti_g = st.session_state.assenze_per_giorno.get(giorno_selezionato, [])
            is_ass = any(a.get('Docente') == s_doc and s_ora in a.get('Ore', []) for a in assenti_g)
            if is_ass:
                row['Stato Sostituto'] = "🚨 ASSENTE (Da Riassegnare)"
            else:
                row['Stato Sostituto'] = "🟢 Confermato"
    ...
    if st.session_state.registro:
        ...
        for row in reg_completo_sorted:
            s_doc = row.get('Docente Sostituto')
            s_ora = row.get('Ora')
            g_row = row.get('Giorno')
            assenti_g = st.session_state.assenze_per_giorno.get(g_row, [])
            is_ass = any(a.get('Docente') == s_doc and s_ora in a.get('Ore', []) for a in assenti_g)
            if is_ass:
                row['Stato Sostituto'] = "🚨 ASSENTE (Da Riassegnare)"
            else:
