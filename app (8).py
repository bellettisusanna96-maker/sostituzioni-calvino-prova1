...
# Inizializzazione Session State con persistenza
...
def is_docente_assente_in_ora(docente, giorno, ora):
    """Verifica se un docente risulta assente nel giorno e ora specificati (inclusa assenza tutto il giorno)."""
    if not docente or str(docente).strip() in ['', '🔴 IN SOSPESO', 'nan', 'None']:
        return False
    doc_clean = str(docente).strip().upper()
    for item in st.session_state.get('assenze_per_giorno', {}).get(giorno, []):
...
def get_conflitti_sostituti_assenti():
    """Restituisce l'elenco di tutte le supplenze assegnate a docenti che risultano poi ASSENTI."""
    conflitti = []
    for reg in st.session_state.get('registro', []):
        ...
        cls = reg.get('Classe')
        if is_docente_assente_in_ora(sost, g, o):
            conflitti.append({
                'Giorno': g,
                'Ora': o,
                'Classe': cls,
                'Docente Assente Iniziale': d_ass,
...
# Riquadro di controllo stato memoria
tot_ass_sett = sum(len(v) for v in st.session_state.assenze_per_giorno.values())
tot_reg_sett = len(st.session_state.registro)
conflitti_attuali = get_conflitti_sostituti_assenti()

st.sidebar.info(f"📊 **Stato In Memoria:**\n- Assenze registrate: `{tot_ass_sett}`\n- Sostituzioni confermate: `{tot_reg_sett}`" + (f"\n- 🚨 Conflitti attivi: `{len(conflitti_attuali)}`" if conflitti_attuali else ""))
...
# ==============================================================================
# BANNER DI ALLERTA GLOBALE PER CONFLITTI SUPPLENTI ASSENTI
# ==============================================================================
if conflitti_attuali:
    st.error(f"🚨 **ALLERTA CRITICA ({len(conflitti_attuali)} CONFLITTO/I RILEVATO/I):** Ci sono docenti assegnati in supplenza che risultano **ASSENTI**!")
    for c in conflitti_attuali:
        st.markdown(f"• 🔴 **{c['Giorno']} - {c['Ora']}** | Classe `{c['Classe']}`: il supplente **{c['Supplente Assente']}** risulta **ASSENTE**! *(Docente sostituito: {c['Docente Assente Iniziale']})*")
    st.markdown("---")
...
with tab_assenze:
    ...
    # Verifica se il docente selezionato è attualmente assegnato come supplente per qualcuno in questo giorno
    sost_gia_assegnato = [
        r for r in st.session_state.registro 
        if r.get('Giorno') == giorno_selezionato and r.get('Docente Sostituto') == doc_ass_sel
    ]
    if sost_gia_assegnato:
        st.error(f"⚠️ **ATTENZIONE:** Il docente **{doc_ass_sel}** è attualmente registrato come **SUPPLENTE** per {len(sost_gia_assegnato)} classe/i il {giorno_selezionato}:")
        for s in sost_gia_assegnato:
            st.markdown(f"  - **{s['Ora']}** | Classe `{s['Classe']}` (in sostituzione di *{s['Docente Assente']}*)")
        st.warning("Se inserisci questo docente tra gli assenti, quelle supplenze rimarranno SCOPERTE e dovranno essere riassegnate nel TAB 2!")
    ...
    with col_add3:
        ...
        if tipo_assenza == "🔴 Tutto il Giorno":
            ore_selezionate_ass = list(ORE)  # L'assenza tutto il giorno copre TUTTE le 7 ore
            if not ore_lezione_doc:
                st.warning(f"Nessuna lezione propria in orario per **{doc_ass_sel}** il **{giorno_selezionato}** (risulterà assente per l'intera giornata).")
                ...
                msg_disp_note = f"\n\n*(Nota: {len(ore_disp_doc)} ora/e a disposizione non richiedono sostituto e sono escluse dal conteggio classi)*" if ore_disp_doc else ""
            ...
            if not ore_lezione_doc:
                st.warning("Nessuna lezione propria in orario per questo docente.")
                ore_selezionate_ass = st.multiselect("Seleziona le ore di assenza dal totale:", options=ORE, default=ORE[:1])
            else:
                ore_selezionate_ass = st.multiselect(
                    "Seleziona le ore di Permesso / Assenza:",
                    options=ORE,
                    default=ore_lezione_doc,
                    format_func=lambda s: f"{s} (Classe {ore_dict.get(s, 'Nessuna')})" if ore_dict.get(s, '').upper() not in ['DISP', 'D', 'DISPOSIZIONE', ''] else f"{s} (DISP / Libero)"
                )
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Controlla se questo docente era assegnato come supplente
            era_supplente = any(r.get('Giorno') == giorno_selezionato and r.get('Docente Sostituto') == item['Docente'] for r in st.session_state.registro)
...
with tab_sostituzioni:
    ...
    # Calcolo di TUTTI i docenti assenti per ciascuna ora specifica (inclusi gli assenti Tutto il Giorno)
    ...
    for item in st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        if item.get('Tipo') == "🔴 Tutto il Giorno":
            ...
            for slot in item.get('Ore', []):
                docenti_assenti_per_ora[slot].add(d_nome)
            
    # Calcolo di TUTTI i docenti già assegnati come sostituti per ciascuna ora (solo se non sono assenti)
    ...
    for reg in st.session_state.registro:
        if reg.get('Giorno') == giorno_selezionato:
            s_ora = reg.get('Ora')
            s_sost = reg.get('Docente Sostituto')
            # Se il sostituto NON è assente, lo consideriamo occupato
            if not is_docente_assente_in_ora(s_sost, giorno_selezionato, s_ora):
                if s_ora in docenti_sostituti_occupati_per_ora:
                    docenti_sostituti_occupati_per_ora[s_ora].add(s_sost)
                else:
                    docenti_sostituti_occupati_per_ora[s_ora] = {s_sost}
    ...
    for item in st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        ore_da_verificare = list(ORE) if item.get('Tipo') == "🔴 Tutto il Giorno" else item.get('Ore', [])
        
        for slot in ore_da_verificare:
            ...
            if cls_val and cls_val.upper() != 'NAN' and cls_val != '':
                if cls_val.upper() in ['DISP', 'D', 'DISPOSIZIONE']:
                    ...
                    # Verifica se il sostituto assegnato risulta assente
                    is_sost_assente = is_docente_assente_in_ora(sostituto_assegnato, giorno_selezionato, slot)
                    ...
                    classi_scoperte_lista.append({
                        ...
                        'Sostituto Assente': is_sost_assente
                    })
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            ...
            is_sost_ass = row_s.get('Sostituto Assente', False)
            
            if is_sost_ass:
                tc5.markdown(f"🚨 **CONFLITTO: {sost_val} (ASSENTE!)**")
                if tc6.button("❌", key=f"btn_del_inline_conf_{idx_row}_{row_s['Ora']}", help=f"Rimuovi {sost_val} assente"):
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            ...
            if is_sost_ass:
                stato_icon = "🚨 CONFLITTO"
                label_txt = f"{stato_icon} {row_s['Ora']} | Assente: {row_s['Docente Assente']} | Classe: {row_s['Classe / Slot']} | Supplente ASSENTE: {sost_val}"
            elif sost_val != "🔴 IN SOSPESO":
                stato_icon = "🟢"
                label_txt = f"{stato_icon} {row_s['Ora']} | Assente: {row_s['Docente Assente']} | Classe: {row_s['Classe / Slot']} | Sostituto: {sost_val}"
            else:
                stato_icon = "🔴"
                label_txt = f"{stato_icon} {row_s['Ora']} | Assente: {row_s['Docente Assente']} | Classe: {row_s['Classe / Slot']} | Sostituto: IN SOSPESO"
                
            opzioni_scoperte.append(label_txt)
        ...
        is_sost_corrente_assente = target_slot.get('Sostituto Assente', False)
        
        if is_sost_corrente_assente:
            st.error(f"🚨 **EMERGENZA:** Il supplente **{sostituto_corrente}** precedentemente assegnato risulta **ASSENTE**! Scegli un nuovo sostituto tra i candidati disponibili qui sotto e clicca su 'Conferma Riassegnazione'.")
        else:
        ...
        for idx, r in df_schedule.iterrows():
            ...
            # FILTRO ESSENZIALE: Escludi chi non è in servizio il giorno selezionato
        ...
        if all_cands:
            ...
            btn_label = "✅ Conferma Riassegnazione Sostituto" if is_sost_corrente_assente else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        # Aggiunta colonna visiva Stato Sostituto
        for r in reg_giorno_sorted:
            s_sost = r.get('Docente Sostituto')
            if is_docente_assente_in_ora(s_sost, r.get('Giorno'), r.get('Ora')):
                r['Stato Sostituto'] = f"🚨 ASSENTE ({s_sost})"
            else:
                r['Stato Sostituto'] = "🟢 Presente"
