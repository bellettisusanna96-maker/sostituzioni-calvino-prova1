...
def is_docente_assente(docente_nome, giorno, ora=None):
    """
    Rileva se un docente è assente nel giorno specificato ed eventualmente nell'ora specificata.
    Se tipo_assenza è '🔴 Tutto il Giorno', il docente risulta assente per tutte le ore.
    """
    if not docente_nome or docente_nome == "🔴 IN SOSPESO":
        return False
        
    docente_clean = str(docente_nome).strip().upper()
    for item in st.session_state.assenze_per_giorno.get(giorno, []):
        if str(item.get('Docente', '')).strip().upper() == docente_clean:
            tipo = item.get('Tipo', '')
            if tipo == "🔴 Tutto il Giorno":
                return True
            if ora:
                return ora in item.get('Ore', [])
            return True
    return False

def get_conflitti_sostituti(giorno_filtro=None):
    """
    Restituisce l'elenco delle sostituzioni assegnate in cui il docente sostituto risulta ASSENTE.
    """
    conflitti = []
    for reg in st.session_state.registro:
        g = reg.get('Giorno')
        if giorno_filtro and g != giorno_filtro:
            continue
        s_doc = reg.get('Docente Sostituto')
        s_ora = reg.get('Ora')
        if is_docente_assente(s_doc, g, s_ora):
            conflitti.append({
                'Giorno': g,
                'Ora': s_ora,
                'Classe': reg.get('Classe'),
                'Docente Assente': reg.get('Docente Assente'),
                'Supplente Assente': s_doc
            })
    return conflitti
...
# Riquadro di stato sessione
tot_ass_count = sum(len(v) for v in st.session_state.assenze_per_giorno.values())
tot_reg_count = len(st.session_state.registro)
st.sidebar.info(f"📊 **Stato In Dati:**\n- Assenze registrate: `{tot_ass_count}`\n- Sostituzioni confermate: `{tot_reg_count}`")
...
# Controllo globale conflitti
tutti_conflitti = get_conflitti_sostituti()
if tutti_conflitti:
    c_msg_lines = []
    for c in tutti_conflitti:
        c_msg_lines.append(f"• **{c['Giorno']}** ({c['Ora']}) - Classe **{c['Classe']}** (Docente originario: *{c['Docente Assente']}*): Il supplente **{c['Supplente Assente']}** risulta ora **ASSENTE**!")
    st.error(f"🚨 **ALLERTA SOSPESA CRITICA ({len(tutti_conflitti)} CONFLITTO/I):** Ci sono supplenze assegnate a docenti che sono stati segnati come ASSENTI!\n\n" + "\n".join(c_msg_lines) + "\n\n👉 **Vai nel Tab 2 per riassegnare un nuovo sostituto.**")
...
with tab_assenze:
    ...
    # Controlla se il docente selezionato era già stato inserito come SUPPLENTE per quel giorno
    ...
    if supplenze_gia_assegnate:
        dettaglio_supp = [f"• **{r['Ora']}**: Classe `{r['Classe']}` (Docente originario: {r['Docente Assente']})" for r in supplenze_gia_assegnate]
        st.error(f"🚨 **ATTENZIONE:** Il docente **{doc_ass_sel}** era già stato assegnato come **SUPPLENTE** il **{giorno_selezionato}** per:\n\n" + "\n".join(dettaglio_supp) + "\n\nSe lo inserisci tra gli assenti, la supplenza risulterà scoperta e dovrai riassegnarla nel TAB 2.")
    ...
    with col_add3:
        ...
        if tipo_assenza == "🔴 Tutto il Giorno":
            # Per 'Tutto il giorno', il docente è assente per TUTTE e 7 le ore della giornata (inclusi buchi/disposizioni)
            ore_selezionate_ass = list(ORE)
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Segnalazione visiva se il docente era anche registrato come sostituto
            era_sostituto = any(r.get('Giorno') == giorno_selezionato and r.get('Docente Sostituto') == item['Docente'] for r in st.session_state.registro)
            note_sost = " ⚠️ *(Era registrato come supplente!)*" if era_sostituto else ""
            
            tc1.markdown(f"👤 **{item['Docente']}**{note_sost}")
...
with tab_sostituzioni:
    ...
    # Calcolo di TUTTI i docenti assenti per ciascuna ora specifica
    ...
    for item in st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # Scorriamo le ore in cui il docente assente ha effettivamente lezioni proprie
        for slot in ORE:
            slot_key = f"{giorno_selezionato}_{slot}"
            cls_val = str(r_info.get(slot_key, '')).strip()
            
            # Verifichiamo se questa ora rientra tra le ore di assenza registrate
            is_assente_in_slot = is_docente_assente(d_nome, giorno_selezionato, slot)
            if not is_assente_in_slot:
                continue
            ...
            if cls_val and cls_val.upper() != 'NAN' and cls_val != '':
                if cls_val.upper() in ['DISP', 'D', 'DISPOSIZIONE']:
                    ...
                    # Controlla se il sostituto attualmente assegnato è a sua volta assente
                    is_sost_assente = is_docente_assente(sostituto_assegnato, giorno_selezionato, slot) if sostituto_assegnato else False
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            ...
            is_conf = row_s['Sostituto Assente']
            
            if is_conf:
                tc5.markdown(f"🚨 **{sost_val} (ASSENTE!)**")
                if tc6.button("❌", key=f"btn_del_inline_{idx_row}_{row_s['Ora']}", help=f"Rimuovi sostituto assente {sost_val}"):
                    ...
                    save_persistent_data()
                    st.success(f"Rimosso sostituto assente **{sost_val}**!")
                    st.rerun()
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            if row_s['Sostituto Assente']:
                stato_icon = "🚨 CONFLITTO (ASSENTE):"
        ...
        is_current_sost_assente = target_slot['Sostituto Assente']
        
        if is_current_sost_assente:
            st.error(f"🚨 **ALLERTA CONFLITTO:** Il sostituto precedentemente assegnato (**{sostituto_corrente}**) risulta **ASSENTE** per {giorno_selezionato} ({ora_target}). Seleziona un nuovo docente tra i candidati disponibili qui sotto per coprire la classe **{classe_target}**.")
        else:
        ...
        for idx, r in df_schedule.iterrows():
            ...
            # Se il docente è assente a quell'ora (o tutto il giorno), escludilo!
            if is_docente_assente(d_nome, giorno_selezionato, ora_target):
                continue
        ...
        if all_cands:
            ...
            btn_label = "✅ Riassegna Nuovo Sostituto" if is_current_sost_assente else "✅ Conferma Assegnazione Sostituto"
...
with tab_registro:
    ...
    if reg_giorno:
        ...
        # Aggiunta colonna di stato per evidenziare eventuali supplenti assenti
        for r in reg_giorno_sorted:
            if is_docente_assente(r.get('Docente Sostituto'), r.get('Giorno'), r.get('Ora')):
                r['Stato Sostituto'] = '🚨 ASSENTE (Da Riassegnare)'
            else:
                r['Stato Sostituto'] = '🟢 Confermato'
