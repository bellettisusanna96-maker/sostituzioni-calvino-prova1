...
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Gestione Settimanale e Storico Assenze (A.S. 2026/2027 - v19)")
    ...
    with col_add3:
        if tipo_assenza == "🔴 Tutto il Giorno":
            ...
            if not ore_lezione_doc:
                ...
                dettaglio_list = [f"• **{s}**: Classe `{ore_dict[s]}`" if ore_dict[s].upper() not in ['DISP', 'D', 'DISPOSIZIONE'] else f"• **{s}**: `DISP`" for s in ore_lezione_doc]
                st.info(f"Verranno inserite tutte le **{len(ore_selezionate_ass)} ore** del docente per **{giorno_selezionato}**:\n\n" + "\n".join(dettaglio_list))
        else:
            if not ore_lezione_doc:
                ...
                ore_selezionate_ass = st.multiselect(
                    ...
                    format_func=lambda s: f"{s} (Classe {ore_dict[s]})" if ore_dict[s].upper() not in ['DISP', 'D', 'DISPOSIZIONE'] else f"{s} (DISP)"
                )
