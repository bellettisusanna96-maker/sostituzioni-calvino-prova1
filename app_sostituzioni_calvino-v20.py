...
with tab_assenze:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        # Intestazione tabella assenti con pulsante X in linea
        th1, th2, th3, th4, th5 = st.columns([2.5, 2.0, 2.0, 2.0, 0.8])
        th1.markdown("**Docente Assente**")
        th2.markdown("**Materia**")
        th3.markdown("**Tipologia Assenza**")
        th4.markdown("**Ore Coinvolte**")
        th5.markdown("**Azione**")
        st.markdown("---")
        
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            tc1, tc2, tc3, tc4, tc5 = st.columns([2.5, 2.0, 2.0, 2.0, 0.8])
            tc1.markdown(f"👤 **{item['Docente']}**")
            tc2.write(item['Materia'])
            tc3.write(item['Tipo'])
            ore_str = ", ".join([o.split(" ")[0] for o in item['Ore']]) if item['Ore'] else "Nessuna"
            tc4.write(ore_str)
            if tc5.button("❌", key=f"btn_remove_inline_tab1_{idx}_{item['Docente']}", help=f"Rimuovi {item['Docente']} dagli assenti"):
