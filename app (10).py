...
### Gestione dinamica assenze per giorno (senza reset al cambio giorno)
...
with tab_assenze:
    ...
    with col_add3:
        ...
        # AVVISO IN EVIDENZA SE IL DOCENTE È GIÀ STATO ASSEGNATO COME SUPPLENTE
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        # Intestazione tabella assenti con Indirizzo e pulsante X in linea
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            # Filtra le ore escludendo le Disposizioni (DISP) per la visualizzazione delle ore coinvolte
    ...
    for g in GIORNI:
        for item in st.session_state.assenze_per_giorno.get(g, []):
            ...
            # Filtra le ore per mostrare solo le classi effettive da sostituire (escludendo le Disposizioni - DISP)
            ore_classe_sett = [
                o for o in item['Ore'] 
...
with tab_sostituzioni:
    ...
    # Generazione Elenco Completo Classi Scoperte e Tracciamento DISP Assenti
    ...
    if not st.session_state.assenze_per_giorno[giorno_selezionato]:
        ...
        # Intestazione tabella con pulsante X azione in linea
        ...
        # Controllo conflitti generati da sostituti risultati poi assenti
        conflitti_sostituti_assenti = []
        for row_check in classi_scoperte_lista:
            s_check = row_check['Sostituto Assegnato']
            if s_check and s_check != "🔴 IN SOSPESO":
                for item_ass in st.session_state.assenze_per_giorno.get(giorno_selezionato, []):
                    if item_ass['Docente'] == s_check:
                        if item_ass.get('Tipo') == "🔴 Tutto il Giorno" or row_check['Ora'] in item_ass.get('Ore', []):
                            conflitti_sostituti_assenti.append({
                                'Ora': row_check['Ora'],
                                'Classe': row_check['Classe / Slot'],
                                'Docente Assente Iniziale': row_check['Docente Assente'],
                                'Supplente Assente': s_check
                            })

        if conflitti_sostituti_assenti:
            dettagli_conf = [
                f"• **{c['Ora']}** - Classe `{c['Classe']}`: Supplente **{c['Supplente Assente']}** risulta ora **ASSENTE** (doverva sostituire *{c['Docente Assente Iniziale']}*)"
                for c in conflitti_sostituti_assenti
            ]
            st.error(
                f"🚨 **ALLERTA SOSPESA CRITICA ({len(conflitti_sostituti_assenti)} CONFLITTO/I):**\n\n"
                + "\n".join(dettagli_conf) +
                "\n\n⚠️ *Riassegna un nuovo sostituto utilizzando il menu sottostante.*"
            )
        ...
        for idx_row, row_s in enumerate(classi_scoperte_lista):
            ...
            # Verifichiamo se il sostituto assegnato è nel frattempo diventato assente
            sostituto_e_assente = False
            if sost_val and sost_val != "🔴 IN SOSPESO":
                for item_ass in st.session_state.assenze_per_giorno.get(giorno_selezionato, []):
                    if item_ass['Docente'] == sost_val:
                        if item_ass.get('Tipo') == "🔴 Tutto il Giorno" or row_s['Ora'] in item_ass.get('Ore', []):
                            sostituto_e_assente = True
                            break

            if sost_val != "🔴 IN SOSPESO":
                if sostituto_e_assente:
        ...
        for idx, row_s in enumerate(classi_scoperte_lista):
            ...
            s_assente = False
            if s_val and s_val != "🔴 IN SOSPESO":
                for item_ass in st.session_state.assenze_per_giorno.get(giorno_selezionato, []):
                    if item_ass['Docente'] == s_val:
                        if item_ass.get('Tipo') == "🔴 Tutto il Giorno" or row_s['Ora'] in item_ass.get('Ore', []):
                            s_assente = True
                            break
            if s_assente:
                stato_icon = "🚨 CONFLITTO (Sostituto Assente!)"
        ...
        sost_target_assente = False
        if sostituto_corrente and sostituto_corrente != "🔴 IN SOSPESO":
            for item_ass in st.session_state.assenze_per_giorno.get(giorno_selezionato, []):
                if item_ass['Docente'] == sostituto_corrente:
                    if item_ass.get('Tipo') == "🔴 Tutto il Giorno" or ora_target in item_ass.get('Ore', []):
                        sost_target_assente = True
                        break

        if sost_target_assente:
            st.error(f"🚨 **CONFLITTO CRITICO:** Il sostituto assegnato (**{sostituto_corrente}**) è risultato **ASSENTE** il **{giorno_selezionato}**! Seleziona un nuovo docente qui sotto per riassegnare la supplenza.")
        else:
        ...
        for idx, r in df_schedule.iterrows():
            ...
            # FILTRO ESSENZIALE: Escludi chi non è in servizio il giorno selezionato (nessuna classe né DISP in nessuna ora del giorno)
            ...
            # Riconoscimento rigoroso delle variazioni di DISPOSIZIONE ('d', 'D', 'DISP', 'disp', 'DISPOSIZIONE')
            is_disp = val_ora_upper in ['D', 'DISP', 'DISPOSIZIONE']
            
            # Se ha lezione con una classe in quell'ora (e non è in disposizione), non è disponibile
            if val_ora and not is_disp and val_ora_upper != 'NAN' and (val_ora in CLASSES_INFO or val_ora_upper not in ['NAN', 'NONE', '']):
                # Se è impegnato in una classe
            ...
            if is_disp:
                ...
                # Il docente è libero nell'ora target (casella vuota), ma sappiamo che è in servizio nel giorno
                ora_idx = ORE.index(ora_target) if ora_target in ORE else 0
                
                # Verifichiamo lezioni/attività prima e dopo nell'arco della giornata
                has_before = any(
        ...
        # Ordinamento tassativo a 10 livelli secondo le priorità richieste
