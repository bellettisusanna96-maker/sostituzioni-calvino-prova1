...
st.subheader("Sistema Automatico Gestione Sostituzioni e Supplenze - Modulo Gestione Settimanale e Storico Assenze (A.S. 2026/2027 - v16)")
...
def sort_registro_chronological(records):
    def sort_key(r):
        g_val = r.get('Giorno', '')
        ...
        return (g_idx, o_idx, str(r.get('Classe', '')), str(r.get('Docente Assente', '')))
...
with tab_assenze:
    ...
    if st.session_state.assenze_per_giorno[giorno_selezionato]:
        table_data = []
        for idx, item in enumerate(st.session_state.assenze_per_giorno[giorno_selezionato]):
        ...
        for idx, item in enumerate(list(st.session_state.assenze_per_giorno[giorno_selezionato])):
            ...
            with c4:
                if st.button(f"❌ Rimuovi", key=f"btn_remove_{idx}_{item['Docente']}"):
                    ...
                    st.session_state.assenze_per_giorno[giorno_selezionato] = [x for x in st.session_state.assenze_per_giorno[giorno_selezionato] if x['Docente'] != doc_removed]
...
with tab_registro:
    ...
    if reg_giorno:
        reg_giorno_sorted = sort_registro_chronological(reg_giorno)
    ...
    st.markdown("---")
    st.subheader("📜 Storico Completo Sessione (Tutti i Giorni — Ordinato per Giorno e Ora)")
    if st.session_state.registro:
        reg_completo_sorted = sort_registro_chronological(st.session_state.registro)
